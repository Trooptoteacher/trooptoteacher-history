...
Maps each SSP -> the organizers that build it -> when/where to use it in Unit 7.
...
SSP = [
    ...
    ("SSP.02", "Critically Examine Sources", "red",
     ...
     ["HIPPO", "C&#8209;E&#8209;R"],
     "Any primary source in US.59&ndash;US.70 &mdash; a Truman Doctrine speech, a &lsquo;duck and cover&rsquo; film, or a Cronkite broadcast."),
    ("SSP.03", "Synthesize Data", "gold",
     ...
     "US.59 (U.S. vs. USSR) &amp; US.67 (Kennedy, Johnson &amp; Nixon in Vietnam) &mdash; holding many cases against one yardstick."),
    ("SSP.04", "Construct &amp; Communicate with Evidence", "navy",
     ...
     "US.63 (brinkmanship vs. peaceful coexistence) &amp; US.69 (the U.S.&ndash;USSR space race) &mdash; compare two."),
    ("SSP.05", "Develop Historical Awareness", "red",
     ...
     "US.62 (the Korean War&rsquo;s course) &amp; US.70 (d&eacute;tente &rarr; the fall of the Berlin Wall)."),
    ("SSP.06", "Develop Geographic Awareness", "gold",
     "Read place, region &amp; spatial patterns; connect geography to events.",
     ["Conflict map", "&#9733; Tennessee Connection"],
     "US.62 (Korea) &amp; US.66 (Vietnam) battle geography &mdash; and every Tennessee Connection (Oak Ridge, Fort Campbell)."),
]
...
<div class="page">
  <div class="cover">
    <div class="l">
      ...
      <div class="sub">Which organizer builds which practice &mdash; and where it lives in Unit 7</div>
    </div>
  ...
  <div class="stbl">
    <div class="sh"><div>Practice</div><div>What students do</div><div>Build it with</div><div>Use it when &middot; where in Unit 7</div></div>
{chr(10).join(rows)}
  </div>
