# -*- coding: utf-8 -*-
"""Full inventory + standards crosswalk for the World History question bank.
...
def findkey(o,code):
    if isinstance(o,dict):
        ...
        for x in o:
            r=findkey(x,code)
            if r: return r
src=json.load(open(os.path.join(ROOT,"05_STANDARDS_ALIGNMENT","world_history_standards_source.json"),encoding="utf-8"))
CODES=sorted({i["standard"] for i in items},key=lambda s:int(re.match(r"W\.(\d+)",s).group(1)))
for c in CODES:
    r=findkey(src,c) or {}
...
L=["# World History Question Bank — Full Inventory & Standards Crosswalk","",
   f"**World History & Geography** (World History Hack edition) · Tennessee World History & Geography (W).",
   f"Total items: **{len(items)}** across **{len(CODES)} standards** (W.01–W.89).",
...
L+=["","---","_Generated by build_inventory.py from world_history_question_bank.json. "
