...
function H(t,l,{brk=false}={}){const m={1:HeadingLevel.HEADING_1,2:HeadingLevel.HEADING_2,3:HeadingLevel.HEADING_3};
  return new Paragraph({heading:m[l],pageBreakBefore:brk,spacing:{before:l===1?220:150,after:90},keepNext:true,children:[Rr(t,{s:l===1?36:l===2?28:24,b:true,c:l===3?RED:NAVY})]});}
...
function blankVocabStudio(){return [
  H('Blank Vocabulary Studio — Frayer (printable)',2,{brk:true}),
...
const body=[
  ...
  H('Student (Lean) Deck — section map',2),
  P(Rr('The Student (Lean) Deck is organized into one titled section per standard, in the order below; each slide shows its standard code. Every section runs the same beats — opener → hook + key terms → primary source → Cornell cues → check for understanding — so students take Cornell Notes on the Universal Front as you move through it.',{s:22})),
  dataTable(['Standard','Deck section','Section beats'],
    C.order.map(c=>[c,C.standards[c].title,'Opener · Hook + Vocab · Primary Source · Cornell Cues · CFU']),[1421,5449,2922]),
  ...
  H('9. Assessment Source & Item Metadata',1,{brk:true}),
  ...
  H('11. How CAST UDL 3.0 Is Visible in Unit 1',1,{brk:true}),
  ...
  H('12. Printable Blank Templates',1,{brk:true}),
  ...
  H('13. Social Studies Practices (SSP) Crosswalk',1,{brk:true}),
  ...
  H('14. Standard Dimension Coverage (C · E · G · H · P · T · TCA)',1,{brk:true}),
  ...
  H('15. Constructed Response (CER) — 6-Point Rubric',1,{brk:true}),
  ...
  H('16. Exit Ticket Answer Keys + What’s Next (reteach)',1,{brk:true}),
