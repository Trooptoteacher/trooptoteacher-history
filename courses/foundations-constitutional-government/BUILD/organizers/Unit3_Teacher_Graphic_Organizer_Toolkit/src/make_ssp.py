# -*- coding: utf-8 -*-
"""Page 2: SSP Crosswalk (TN Social Studies Practice -> organizers -> where in Unit 3).
...
# (code, short practice (from verbatim SSP), organizers that build it, where in Unit 3, color)
ROWS = [
    ("SSP.01", "Collect data &amp; information from a variety of primary and secondary sources.",
     ...
     "GC.35 citizenship (Venn); GC.14 my delegation (Concept Web / 5 Ws); K-W-L to open.", "gold"),
    ("SSP.02", "Critically examine a source: purpose, point of view, evidence vs. assertion, bias.",
     "HIPPO &middot; C-E-R",
     "GC.15 powers &amp; McCulloch (source analysis / HIPPO on the opinion).", "red"),
    ("SSP.03", "Synthesize data from several sources; compare accounts; frame new questions.",
     ...
     "GC.10 House vs. Senate (Matrix); GC.35 natural-born vs. naturalized (Venn).", "navy"),
    ("SSP.04", "Construct &amp; communicate arguments citing supporting evidence.",
     "C-E-R &middot; T-Chart &middot; Problem&ndash;Solution",
     "GC.12 House vs. Senate leadership (T-Chart); C-E-R on implied powers.", "navy"),
    ("SSP.05", "Develop historical awareness: cause, context, continuity &amp; change over time.",
     ...
     "GC.11 census &rarr; redistricting (Cause &amp; Effect); GC.13 how a bill becomes law (Sequence).", "red"),
...
HTML = f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><title>SSP Crosswalk &mdash; Unit 3</title>
<style>{SHARED_CSS}
{CSS}</style></head>
<body>
<div class="page">
  ...
  This crosswalk maps each practice to the organizers that develop it &mdash; and shows <b>where</b> it lives in Unit 3 &mdash;
  so a graphic organizer is never busywork: it is deliberate practice toward a standard.</div>

  <div class="qtbl">
    <div class="qh"><div>Practice</div><div>What students do (TN SSP)</div><div>Organizers that build it</div><div>Where in Unit 3</div></div>
