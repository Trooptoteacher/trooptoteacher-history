# -*- coding: utf-8 -*-
"""Page 2: SSP Crosswalk (TN Social Studies Practice -> organizers -> where in Unit 6).
...
# (code, short practice (from verbatim SSP), organizers that build it, where in Unit 6, color)
ROWS = [
    ("SSP.01", "Collect data &amp; information from a variety of primary and secondary sources.",
     ...
     "GC.27 expanding rights (Main Idea & Details); K-W-L to open.", "gold"),
    ("SSP.02", "Critically examine a source: purpose, point of view, evidence vs. assertion, bias.",
     "HIPPO &middot; C-E-R",
     "GC.23 First Amendment cases (HIPPO / analyze a source or ruling).", "red"),
    ("SSP.03", "Synthesize data from several sources; compare accounts; frame new questions.",
     ...
     "GC.23 First Amendment cases (Matrix, case × freedom × ruling).", "navy"),
    ("SSP.04", "Construct &amp; communicate arguments citing supporting evidence.",
     "C-E-R &middot; T-Chart &middot; Problem&ndash;Solution",
     "GC.25 rights of the accused (Concept Web); C-E-R on a liberties claim.", "navy"),
    ("SSP.05", "Develop historical awareness: cause, context, continuity &amp; change over time.",
     ...
     "GC.24 Heller → McDonald (Cause & Effect); GC.26 Plessy → Brown → Obergefell (Timeline).", "red"),
...
HTML = f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><title>SSP Crosswalk &mdash; Unit 6</title>
<style>{SHARED_CSS}
{CSS}</style></head>
<body>
<div class="page">
  ...
  This crosswalk maps each practice to the organizers that develop it &mdash; and shows <b>where</b> it lives in Unit 6 &mdash;
  so a graphic organizer is never busywork: it is deliberate practice toward a standard.</div>

  <div class="qtbl">
    <div class="qh"><div>Practice</div><div>What students do (TN SSP)</div><div>Organizers that build it</div><div>Where in Unit 6</div></div>
