# -*- coding: utf-8 -*-
"""Page 2: SSP Crosswalk (TN Social Studies Practice -> organizers -> where in Unit 2).
...
# (code, short practice (from verbatim SSP), organizers that build it, where in Unit 2, color)
ROWS = [
    ("SSP.01", "Collect data &amp; information from a variety of primary and secondary sources.",
     ...
     "GC.31 responsibilities (T-Chart legal vs. expected); K-W-L to open the unit.", "gold"),
    ("SSP.02", "Critically examine a source: purpose, point of view, evidence vs. assertion, bias.",
     "HIPPO &middot; C-E-R",
     "GC.33 media &amp; public opinion (Cause &amp; Effect / HIPPO on a news source).", "red"),
    ("SSP.03", "Synthesize data from several sources; compare accounts; frame new questions.",
     ...
     "GC.34 ways to participate (Concept Web); compare parties &amp; elections (Matrix).", "navy"),
    ("SSP.04", "Construct &amp; communicate arguments citing supporting evidence.",
     "C-E-R &middot; T-Chart &middot; Problem&ndash;Solution",
     "GC.31 responsibilities (T-Chart); C-E-R on a civic-duty claim.", "navy"),
    ("SSP.05", "Develop historical awareness: cause, context, continuity &amp; change over time.",
     ...
     "GC.32 nomination process (Sequence: primary/caucus &rarr; convention &rarr; general election).", "red"),
...
HTML = f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><title>SSP Crosswalk &mdash; Unit 2</title>
<style>{SHARED_CSS}
{CSS}</style></head>
<body>
<div class="page">
  <div class="cover">
    <div class="l">
      ...
      <div class="sub">Unit 2 &middot; Citizen Participation &nbsp;(SSP.01&ndash;SSP.06)</div>
    </div>
    <div class="r">
  ...
  This crosswalk maps each practice to the organizers that develop it &mdash; and shows <b>where</b> it lives in Unit 2 &mdash;
  so a graphic organizer is never busywork: it is deliberate practice toward a standard.</div>

  <div class="qtbl">
    <div class="qh"><div>Practice</div><div>What students do (TN SSP)</div><div>Organizers that build it</div><div>Where in Unit 2</div></div>
