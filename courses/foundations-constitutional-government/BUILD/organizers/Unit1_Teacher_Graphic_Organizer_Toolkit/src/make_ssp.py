# -*- coding: utf-8 -*-
"""Page 2: SSP Crosswalk (TN Social Studies Practice -> organizers -> where in Unit 1).
...
# (code, short practice (from verbatim SSP), organizers that build it, where in Unit 1, color)
ROWS = [
    ("SSP.01", "Collect data &amp; information from a variety of primary and secondary sources.",
     ...
     "GC.05 Preamble (Main Idea &amp; Details); K-W-L to open the unit.", "gold"),
    ("SSP.02", "Critically examine a source: purpose, point of view, evidence vs. assertion, bias.",
     "HIPPO &middot; C-E-R",
     "GC.02 founding documents (HIPPO); blank HIPPO on any primary source.", "red"),
    ("SSP.03", "Synthesize data from several sources; compare accounts; frame new questions.",
     ...
     "GC.01 roots (Concept Web); GC.06 &amp; GC.09 (Venn); GC.08 Bill of Rights (Matrix).", "navy"),
    ("SSP.04", "Construct &amp; communicate arguments citing supporting evidence.",
     "C-E-R &middot; T-Chart &middot; Problem&ndash;Solution",
     "GC.03 Articles &amp; GC.04 Federalists vs. Anti-Federalists (T-Chart); C-E-R blank.", "navy"),
    ("SSP.05", "Develop historical awareness: cause, context, continuity &amp; change over time.",
     ...
     "GC.02 English rights &rarr; Declaration (Cause &amp; Effect); GC.07 amendment process (Sequence).", "red"),
...
HTML = f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><title>SSP Crosswalk &mdash; Unit 1</title>
<style>{SHARED_CSS}
{CSS}</style></head>
<body>
<div class="page">
  <div class="cover">
    <div class="l">
      ...
      <div class="sub">Unit 1 &middot; Foundations of Constitutional Government &nbsp;(SSP.01&ndash;SSP.06)</div>
    </div>
    <div class="r">
  ...
  This crosswalk maps each practice to the organizers that develop it &mdash; and shows <b>where</b> it lives in Unit 1 &mdash;
  so a graphic organizer is never busywork: it is deliberate practice toward a standard.</div>

  <div class="qtbl">
    <div class="qh"><div>Practice</div><div>What students do (TN SSP)</div><div>Organizers that build it</div><div>Where in Unit 1</div></div>
