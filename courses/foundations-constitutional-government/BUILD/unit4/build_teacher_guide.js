...
// ---- course/unit chrome (parameterized) ----
const U=C.unit; const BRAND=(U.brand||'Government Hack'); const BRANDTM=BRAND+'™';
const RANGE=U.standards_range||''; const EQ=U.essential_question||'';
const KEYDIST=C.order.map(c=>AKEY_CFU[c]).join(' · ');

const body=[
  H((U.code||'Unit 1')+' — Teacher How-to-Use & MTSS Guide',1),
  P(Rr((U.title||'')+' · Course Standard · '+RANGE+' · Grades 9–12',{s:22,b:true,c:NAVY})),
  ...
  dataTable(['Criterion','Where it shows'],[
    ['Standards alignment ('+RANGE+')','Standards & SSP Crosswalk; per-standard blocks.'],
...
const footer=new Footer({children:[new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:0},children:[
  Rr(BRANDTM+' · '+(U.code||'Unit 1')+' Teacher Guide   © 2026 TroopToTeacher Technologies LLC   |   Page ',{s:15,c:GREY}),
  new TextRun({children:[PageNumber.CURRENT],size:15,color:GREY,bold:true,font:FONT})]})]});
const doc=new Document({creator:'TroopToTeacher Technologies LLC',title:BRAND+' — '+(U.code||'Unit 1')+' Teacher How-to-Use & MTSS Guide',
