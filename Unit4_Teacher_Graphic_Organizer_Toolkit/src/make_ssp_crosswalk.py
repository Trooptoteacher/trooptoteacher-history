...
Maps each SSP -> the organizers that build it -> when/where to use it in Unit 4.
...
SSP = [
    ...
    ("SSP.02", "Critically Examine Sources", "red",
     ...
     ["HIPPO", "C&#8209;E&#8209;R"],
     "Any primary source in US.28&ndash;US.38 &mdash; a <i>Chicago Defender</i> migrant letter, a 1920s ad, or a Scopes Trial account."),
    ("SSP.03", "Synthesize Data", "gold",
     ...
     "US.30 (country vs. blues) &amp; US.35 (Wells, Garvey &amp; the NAACP) &mdash; holding many cases against one yardstick."),
    ("SSP.04", "Construct &amp; Communicate with Evidence", "navy",
     ...
     "US.34 (women&rsquo;s changes vs. limits) &amp; US.36 (the Scopes Trial &mdash; two sides) &mdash; take a stance."),
    ("SSP.05", "Develop Historical Awareness", "red",
     ...
     "US.28 (push/pull &rarr; the Great Migration) &amp; US.37 (Prohibition&rsquo;s intended vs. actual effects)."),
    ("SSP.06", "Develop Geographic Awareness", "gold",
     "Read place, region &amp; spatial patterns; connect geography to events.",
     ["Migration map", "&#9733; Tennessee Connection"],
     "US.28 (South &rarr; North migration routes) &mdash; and every Tennessee Connection (Opry, Dayton, Pulaski)."),
]
...
<div class="page">
  <div class="cover">
    <div class="l">
      ...
      <div class="sub">Which organizer builds which practice &mdash; and where it lives in Unit 4</div>
    </div>
  ...
  <div class="stbl">
    <div class="sh"><div>Practice</div><div>What students do</div><div>Build it with</div><div>Use it when &middot; where in Unit 4</div></div>
{chr(10).join(rows)}
  </div>
