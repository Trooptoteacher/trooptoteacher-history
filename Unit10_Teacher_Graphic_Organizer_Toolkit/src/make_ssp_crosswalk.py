...
Maps each SSP -> the organizers that build it -> when/where to use it in Unit 10.
...
SSP = [
    ...
    ("SSP.02", "Critically Examine Sources", "red",
     ...
     ["HIPPO", "C&#8209;E&#8209;R"],
     "Any primary source in US.83&ndash;US.95 &mdash; a Watergate hearing clip, a Reagan speech, or an Apollo&#8209;era news broadcast."),
    ("SSP.03", "Synthesize Data", "gold",
     ...
     "US.91 (Clinton&rsquo;s policies) &amp; US.93 (women &amp; minorities in leadership) &mdash; holding many cases against one yardstick."),
    ("SSP.04", "Construct &amp; Communicate with Evidence", "navy",
     ...
     "US.94 (1970s vs. present&#8209;day communication) &mdash; compare two."),
    ("SSP.05", "Develop Historical Awareness", "red",
     ...
     "US.86 (Watergate&rsquo;s unfolding) &amp; US.90 (Bush&rsquo;s foreign&#8209;policy events in order)."),
    ("SSP.06", "Develop Geographic Awareness", "gold",
     "Read place, region &amp; spatial patterns; connect geography to events.",
     ["Modern map", "&#9733; Tennessee Connection"],
     "US.92 (Fort Campbell to Afghanistan) &mdash; and every Tennessee Connection (Tellico Dam, Chattanooga&rsquo;s &lsquo;Gig City&rsquo;)."),
]
...
<div class="page">
  <div class="cover">
    <div class="l">
      ...
      <div class="sub">Which organizer builds which practice &mdash; and where it lives in Unit 10</div>
    </div>
  ...
  <div class="stbl">
    <div class="sh"><div>Practice</div><div>What students do</div><div>Build it with</div><div>Use it when &middot; where in Unit 10</div></div>
{chr(10).join(rows)}
  </div>
