...
- **Still to execute (the one remaining move):** the review-first **skills-only reconciliation PR to
  `main`** promotes the ADOPTs, runs the version guardrail on every 🔁 MERGE, retires the crossed-out,
  applies America 250 + future-ready, and adds the `.claude/skills/SKILLS.md` registry + CI lint.
- **After that:** every one of the next 8 courses builds from this one canonical Platinum set — one
