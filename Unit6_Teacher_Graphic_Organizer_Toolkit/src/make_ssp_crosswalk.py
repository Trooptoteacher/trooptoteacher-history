...
Maps each SSP -> the organizers that build it -> when/where to use it in Unit 6.
...
SSP = [
    ...
    ("SSP.02", "Critically Examine Sources", "red",
     ...
     ["HIPPO", "C&#8209;E&#8209;R"],
     "Any primary source in US.45&ndash;US.58 &mdash; a Pearl Harbor headline, camp-liberation footage, or an internment order."),
    ("SSP.03", "Synthesize Data", "gold",
     ...
     "US.45 (fascism vs. communism) &amp; US.50 (the major battles) &mdash; holding many cases against one yardstick."),
    ("SSP.04", "Construct &amp; Communicate with Evidence", "navy",
     ...
     "US.54 (internment &mdash; security vs. the Constitution) &amp; US.56 (the atomic&#8209;bomb decision) &mdash; take a stance."),
    ("SSP.05", "Develop Historical Awareness", "red",
     ...
     "US.46 (FDR&rsquo;s steps toward war) &amp; US.48 (the road to Pearl Harbor &rarr; U.S. entry)."),
    ("SSP.06", "Develop Geographic Awareness", "gold",
     "Read place, region &amp; spatial patterns; connect geography to events.",
     ["Battle map", "&#9733; Tennessee Connection"],
     "US.50 (Pacific &amp; European battle geography) &mdash; and every Tennessee Connection (Oak Ridge, Fort Campbell)."),
]
...
<div class="page">
  <div class="cover">
    <div class="l">
      ...
      <div class="sub">Which organizer builds which practice &mdash; and where it lives in Unit 6</div>
    </div>
  ...
  <div class="stbl">
    <div class="sh"><div>Practice</div><div>What students do</div><div>Build it with</div><div>Use it when &middot; where in Unit 6</div></div>
{chr(10).join(rows)}
  </div>
