...
Maps each SSP -> the organizers that build it -> when/where to use it in Unit 9.
...
SSP = [
    ...
    ("SSP.02", "Critically Examine Sources", "red",
     ...
     ["HIPPO", "C&#8209;E&#8209;R"],
     "Any primary source in US.78&ndash;US.82 &mdash; the Brown ruling, a &lsquo;I Have a Dream&rsquo; recording, or a sit&#8209;in photograph."),
    ("SSP.03", "Synthesize Data", "gold",
     ...
     "US.81 (the three civil rights laws) &amp; US.82 (three parallel movements) &mdash; holding many cases against one yardstick."),
    ("SSP.04", "Construct &amp; Communicate with Evidence", "navy",
     ...
     "US.79 (advocates vs. opponents) &mdash; two sides, side by side."),
    ("SSP.05", "Develop Historical Awareness", "red",
     ...
     "US.78 (Brown: context &rarr; ruling &rarr; impact) &amp; US.80 (the movement&rsquo;s key events in order)."),
    ("SSP.06", "Develop Geographic Awareness", "gold",
     "Read place, region &amp; spatial patterns; connect geography to events.",
     ["Movement map", "&#9733; Tennessee Connection"],
     "US.80 (Nashville, Clinton &amp; Fayette County) &mdash; and every Tennessee Connection (Highlander, the Nashville sit&#8209;ins)."),
]
...
<div class="page">
  <div class="cover">
    <div class="l">
      ...
      <div class="sub">Which organizer builds which practice &mdash; and where it lives in Unit 9</div>
    </div>
  ...
  <div class="stbl">
    <div class="sh"><div>Practice</div><div>What students do</div><div>Build it with</div><div>Use it when &middot; where in Unit 9</div></div>
{chr(10).join(rows)}
  </div>
