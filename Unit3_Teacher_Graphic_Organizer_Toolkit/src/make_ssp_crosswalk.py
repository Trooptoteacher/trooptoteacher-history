...
Maps each SSP -> the organizers that build it -> when/where to use it in Unit 3.
...
SSP = [
    ...
    ("SSP.02", "Critically Examine Sources", "red",
     ...
     ["HIPPO", "C&#8209;E&#8209;R"],
     "Any primary source in US.19&ndash;US.27 &mdash; a yellow-journalism cartoon, the Zimmermann Telegram, or Wilson&rsquo;s speech."),
    ("SSP.03", "Synthesize Data", "gold",
     ...
     "US.22 (three diplomacies) &amp; US.26 (home front) &mdash; holding many cases against one yardstick."),
    ("SSP.04", "Construct &amp; Communicate with Evidence", "navy",
     ...
     "US.20 (interventionists vs. non&#8209;interventionists) &amp; US.27 (join the League?) &mdash; take a stance."),
    ("SSP.05", "Develop Historical Awareness", "red",
     ...
     "US.21 (war &rarr; new empire) &amp; US.24 (the road to war, 1917)."),
    ("SSP.06", "Develop Geographic Awareness", "gold",
     "Read place, region &amp; spatial patterns; connect geography to events.",
     ["Territory matrix", "&#9733; Tennessee Connection"],
     "US.21 (overseas territories) &amp; US.25 (the Western Front) &mdash; and every Tennessee Connection."),
]
...
<div class="page">
  <div class="cover">
    <div class="l">
      ...
      <div class="sub">Which organizer builds which practice &mdash; and where it lives in Unit 3</div>
    </div>
  ...
  <div class="stbl">
    <div class="sh"><div>Practice</div><div>What students do</div><div>Build it with</div><div>Use it when &middot; where in Unit 3</div></div>
{chr(10).join(rows)}
  </div>
