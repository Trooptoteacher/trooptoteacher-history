...
- **Print-first via the WeasyPrint `print-pipeline` (LOCKED — no `.docx` deliverable).** Author every document (workbook, teacher guide, answer key) through the governed **`print-pipeline`** (`print-pipeline/render.py` + a per-unit content JSON + the **locked `print-contract.css`** Paged-Media contract), which WeasyPrint renders to PDF. **Do NOT produce a `.docx` as the deliverable** (Sean's standing directive). The locked Paged-Media contract owns page breaks, running headers/footers, and page numbers correctly — that is *why* the old ad-hoc "HTML→PDF" ban existed (uncontracted HTML mangled pagination); the contract removes that failure mode, so the ban does not apply to the governed pipeline. The **PDF is the distribution artifact teachers receive and print**; an editable `.docx` MAY be kept as an *optional* author archive but is **never** the delivery path and is not required. (24×36 posters are the only direct-vector exception; they are not documents.) See `00_START_HERE/BUILD_STANDARD.md` §4 + the `print-contract.css` contract.
...
- **Zero blank pages on every rendered PDF; generous notebook-lined writing space visible** on every ruled page (never a single cramped line — owner engine `history-hack-unit-content-build`).
- **Adoption crosswalk + Social Studies Practices present.** The unit ships the Standards Alignment / Adoption Crosswalk built per `history-hack-unit-content-build` (verbatim TDOE standard + **SSP.01–SSP.06** + cross-curricular ELA + DOK coverage + reviewer assurances); the `tn-textbook-adoption-agent` panel runs the formal Schedule F review.
- **UDL 3.0 (CAST, 2024) supports back page present** in the student workbook — the three-principle crosswalk (Engagement / Representation / Action & Expression) + citation, **in addition to** the per-standard NOTES SUPPORTS ladder (design owner `udl-cast-expert`).
- **Accessibility / Rubric F gate.** Delivered PDFs are **tagged PDF/UA** (`/Lang`, Title, `MarkInfo/Marked`, `DisplayDocTitle`) with complete alt text, logical reading order, and **WCAG 2.2 AA** contrast; `accessibility-qc-agent` runs as the **terminal** accessibility gate (Section 508 / ADA Title II) before release.
...
These gates are release-blocking: a unit with any lesson-flow blocker/major, any text-integrity BLOCKER, a Schedule F section/unit under 80% as-built, a blank page, invisible/cramped notebook lines, a missing adoption crosswalk or SSP alignment, a missing UDL 3.0 (CAST 2024) back page, or a failed accessibility (tagged-PDF/UA · WCAG 2.2 AA) gate is **not** platinum and does not ship.
