#!/usr/bin/env python3
"""Derive analysis/unit1_content.json (+ exit tickets + images) for the Unit 2
...
ican = json.load(open(f"{WEB}/ican/unit-1.json"))
vocab = json.load(open(f"{WEB}/vocabulary/unit-1.json"))
psrc = json.load(open(f"{WEB}/primary-sources/unit-1.json"))
dok = {}
for lvl in (1, 2, 3, 4):
    raw = json.load(open(f"{WEB}/questions/unit-1/dok-{lvl}.json"))
...
try:
    _factcards = json.load(open(f"{WEB}/factcards/unit-1.json"))
except Exception:
    _factcards = []
FC_BY_STD = {}
...
for idx, code in enumerate(ORDER):
    ...
    # The West's transformation (US.01) is the era's defining spatial story:
    # the transcontinental railroad and Homestead Act opened the Plains to
    # settlers and displaced Native nations. That standard gets the Geographer's
    # Lens; no generic filler elsewhere.
    GEO_TASKS = {
        "US.01": "The transcontinental railroad and the Homestead Act reshaped the American West. Locate the railroad route and the lands opened to settlement, then explain how geography drove westward expansion — and what it cost the Native nations already living there.",
    }
    GEO_PLACES = {
        "US.01": [
            ["Omaha, Nebraska", "Great Plains", "eastern terminus — the Union Pacific built west from here"],
            ["Promontory Summit, Utah", "Great Basin", "1869 — the golden spike joined the Union Pacific and Central Pacific"],
            ["Sacramento, California", "West Coast", "western terminus — the Central Pacific built east from here"],
            ["The Great Plains", "Interior West", "Homestead Act opened 160-acre plots to settlers; the buffalo and Native nations were pushed out"],
            ["Indian Territory (Oklahoma)", "Southern Plains", "where the federal government forced many displaced Native nations"],
        ],
    }
...
json.dump(content, open(f"{OUT}/unit1_content.json", "w"), ensure_ascii=False, indent=1)
json.dump(exit_tickets, open(f"{OUT}/unit1_exit_tickets.json", "w"), ensure_ascii=False, indent=1)
# NOTE: unit1_images.json is produced by build_unit2_images.py — do NOT clobber it here.
