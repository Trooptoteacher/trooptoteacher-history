// Unit 4 Course Standard STUDENT WORKBOOK — docx-js, EXACT Unit 5 template (no deviation).
...
const C=JSON.parse(fs.readFileSync('analysis/unit4_content.json','utf8'));
const IMG=fs.existsSync('analysis/unit4_images.json')?JSON.parse(fs.readFileSync('analysis/unit4_images.json','utf8')):{};
const EXIT=fs.existsSync('analysis/unit4_exit_tickets.json')?JSON.parse(fs.readFileSync('analysis/unit4_exit_tickets.json','utf8')):{};
...
const HERO={file:'analysis/assets/hero.jpg', w:1021, h:689,
  cap:'Automobiles on a Ford assembly line during the era of mass production, early twentieth century.',
  cred:'Public domain.'};
...
const cover=[
  ...
  P(R('UNIT 4',{s:52,b:true,c:RED}),{align:AlignmentType.CENTER,spacing:{after:60}}),
  P(R('The 1920s, 1920–1929',{s:32,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:120}}),
  P(R('COURSE STANDARD EDITION',{s:26,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:60}}),
  P(R('US.28 – US.38   •   11 Tennessee U.S. History Standards   •   Grades 9–12',{s:22,c:INK}),{align:AlignmentType.CENTER,spacing:{after:140}}),
  ...coverHero(HERO),
  callout('TENNESSEE CONNECTION · THE SCOPES TRIAL & THE GRAND OLE OPRY',['Two of the decade’s biggest stories happened in Tennessee in 1925. In Dayton, the Scopes “Monkey Trial” (US.36) put teacher John Scopes on trial for teaching evolution — Clarence Darrow vs. William Jennings Bryan — in a case broadcast to the nation. That same year in Nashville, WSM radio launched what became the Grand Ole Opry (US.30), making the city the home of country music.']),
...
const front=[
  ...
  P(R('U.S. History Hack™ — Unit 4: The 1920s (1920–1929), Course Standard Edition.',{s:22,b:true})),
  ...
  P(R('Framework stack. This workbook anchors to Tennessee U.S. History standards US.28–US.38 and the Social Studies Practices (SSP.01–SSP.06), and is designed on CAST UDL 3.0 and an MTSS support model.',{s:22})),
...
function block(code){
  ...
  out.push(callout('BIG-QUESTION ORGANIZER — in the 1920s, who shared in the boom and the new freedoms, and who was left out or pushed back?',['Jot quick notes from THIS standard: Who benefited? · Who bore the costs? · Who decided?']));
  out.push(...ruled(2));
...
const back=[
  ...
  callout('TENNESSEE CONNECTION · THE SCOPES TRIAL & THE GRAND OLE OPRY',['Tennessee sat at the center of the 1920s. In the summer of 1925, the little town of Dayton hosted the Scopes “Monkey Trial” (US.36): high-school teacher John Scopes was tried for teaching evolution in violation of the state’s Butler Act, with Clarence Darrow defending and William Jennings Bryan prosecuting. It became the first American trial broadcast live on national radio — the era’s clash between science and religion, and between modern cities and rural tradition, played out in a Tennessee courtroom. That same year, WSM radio in Nashville began the barn-dance program that became the Grand Ole Opry (US.30), and Nashville grew into the home of country music while Memphis’s Beale Street shaped the blues.']),
  ...
  H('The Roaring Twenties — Who Roared, and Who Was Left Out?',1),
  P(R('This section complicates the “Roaring Twenties” narrative: who shared in the boom and the new freedoms, and who was left out or pushed back. It is History Hack-authored instructional synthesis grounded in this unit’s sourced record — not a primary source.',{s:22})),
  ...[['Migration & the Harlem Renaissance','Millions of Black Americans left the Jim Crow South for Northern and Midwestern cities in the Great Migration (US.28), building the Harlem Renaissance’s outpouring of music, literature, and art (US.33) — even while facing discrimination, housing segregation, and violence.'],
    ['A New Consumer Culture','Cars, radio, movies, aviation, and electricity remade daily life (US.29, US.31, US.32); buying on installment credit fueled a consumer boom (US.38). The prosperity reached urban and white Americans far more than farmers, and Black and immigrant workers.'],
    ['Changing Roles for Women','The 19th Amendment and new social freedoms reshaped women’s public lives — the “flapper” became a symbol of the decade (US.34) — though the changes reached some women much more than others.'],
    ['Fear and Backlash','The boom came with reaction: the Red Scare and Palmer Raids, the Sacco-Vanzetti case, and a revived Ku Klux Klan (US.35); Prohibition and the bootlegging it spawned (US.37); and the Scopes Trial’s clash over evolution, science, and religion (US.36).']
  ...
  H('Unit Reflection',2),
  P(R('Return to the unit’s big question: in the 1920s, who shared in the boom and the new freedoms, and who was left out or pushed back? Write a final claim with two pieces of evidence from different standards.',{s:22})),
...
// ================= ASSEMBLE =================
const header=new Header({children:[P(R('U.S. History Hack™ · Unit 4 · Course Standard Edition',{s:16,b:true,c:GOLD}),{align:AlignmentType.RIGHT,spacing:{after:0}})]});
const footer=new Footer({children:[new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:0},children:[
  R('U.S. History Hack™ · Unit 4 (Course Standard)   © 2026 TroopToTeacher Technologies LLC   |   Page ',{s:15,c:GREY}),
  new TextRun({children:[PageNumber.CURRENT],size:15,color:GREY,bold:true,font:FONT})]})]});
const doc=new Document({creator:'TroopToTeacher Technologies LLC',title:'U.S. History Hack — Unit 4 Course Standard Student Workbook (Pilot)',
  ...
  sections:[{properties:{page:{size:{width:12240,height:15840},margin:{top:1152,bottom:1152,left:1296,right:1296,header:720,footer:720}}},
    headers:{default:header},footers:{default:footer},children: SAMPLE ? [P(R(`SAMPLE — Unit 4 · first ${SAMPLE} standards (activities only, ruled-line writing) — for review`,{s:24,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:120}}), ...standards] : [...cover,...front,...standards,...back]}]});
const OUTFILE=SAMPLE?`deliverables/Unit4_SAMPLE_${SAMPLE}standards.docx`:'deliverables/Unit4_Student_Workbook_CourseStandard.docx';
