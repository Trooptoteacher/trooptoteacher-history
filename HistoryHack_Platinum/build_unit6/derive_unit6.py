#!/usr/bin/env python3
"""Derive analysis/unit6_content.json (+ exit tickets + images) for the Unit 2
...
ican = json.load(open(f"{WEB}/ican/unit-6.json"))
vocab = json.load(open(f"{WEB}/vocabulary/unit-6.json"))
psrc = json.load(open(f"{WEB}/primary-sources/unit-6.json"))
dok = {}
for lvl in (1, 2, 3, 4):
    raw = json.load(open(f"{WEB}/questions/unit-6/dok-{lvl}.json"))
...
try:
    _factcards = json.load(open(f"{WEB}/factcards/unit-6.json"))
except Exception:
    _factcards = []
FC_BY_STD = {}
...
for idx, code in enumerate(ORDER):
    ...
    GEO_TASKS = {
        "US.50": "World War II was fought across two enormous theaters. Use the list below to map the major battles in the Pacific and in Europe, then explain the island-hopping and second-front patterns.",
        "US.56": "The atomic bomb was built and used across a handful of secret, far-flung sites. Use the list below to map them, then explain how geography kept the project secret and shaped the war's end.",
    }
    GEO_PLACES = {
        "US.50": [
            ["Midway", "Central Pacific", "1942 naval battle — the turning point in the Pacific"],
            ["Iwo Jima", "Pacific (near Japan)", "1945 island-hopping battle toward the Japanese mainland"],
            ["Okinawa", "Pacific (near Japan)", "1945 — the last major island before Japan"],
            ["Normandy (D-Day)", "France, Western Europe", "1944 Allied invasion — the second front in Europe"],
        ],
        "US.56": [
            ["Oak Ridge, Tennessee", "East Tennessee", "the 'Secret City' — enriched uranium for the bomb"],
            ["Los Alamos, New Mexico", "Desert Southwest", "the lab where the bomb was designed (Oppenheimer)"],
            ["Hanford, Washington", "Pacific Northwest", "produced plutonium for the 'Fat Man' bomb"],
            ["Hiroshima, Japan", "Japan", "first atomic bomb dropped, August 6, 1945"],
            ["Nagasaki, Japan", "Japan", "second atomic bomb dropped, August 9, 1945"],
        ],
    }
...
json.dump(content, open(f"{OUT}/unit6_content.json", "w"), ensure_ascii=False, indent=1)
json.dump(exit_tickets, open(f"{OUT}/unit6_exit_tickets.json", "w"), ensure_ascii=False, indent=1)
# NOTE: unit6_images.json is produced by build_unit2_images.py — do NOT clobber it here.
