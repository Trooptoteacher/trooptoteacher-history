#!/usr/bin/env python3
"""Derive analysis/unit3_content.json (+ exit tickets + images) for the Unit 2
...
ican = json.load(open(f"{WEB}/ican/unit-3.json"))
vocab = json.load(open(f"{WEB}/vocabulary/unit-3.json"))
psrc = json.load(open(f"{WEB}/primary-sources/unit-3.json"))
dok = {}
for lvl in (1, 2, 3, 4):
    raw = json.load(open(f"{WEB}/questions/unit-3/dok-{lvl}.json"))
...
try:
    _factcards = json.load(open(f"{WEB}/factcards/unit-3.json"))
except Exception:
    _factcards = []
FC_BY_STD = {}
...
content = {
    "unit": {
        "code": "Unit 3",
        "title": "Imperialism and World War I (1890–1920)",
        "standards_range": "US.19–US.27",
...
for idx, code in enumerate(ORDER):
    ...
    GEO_TASKS = {
        "US.21": "Map the territories the United States gained or controlled after the Spanish-American War (1898). The places and where they are are listed below — you do NOT have to know them from memory.",
        "US.23": "World War I split Europe into two sides. Use the list below to map the Allied Powers, the Central Powers, and the Western Front where most of the fighting happened.",
    }
    ...
    GEO_PLACES = {
        "US.21": [
            ["Cuba", "Caribbean", "U.S. protectorate after 1898 (Platt Amendment)"],
            ["Puerto Rico", "Caribbean", "became a U.S. territory"],
            ["Philippines", "Pacific / SE Asia", "annexed by the U.S. after the war"],
            ["Guam", "Pacific", "ceded by Spain to the U.S."],
            ["Hawaii", "Pacific", "annexed by the U.S. in 1898"],
        ],
        "US.23": [
            ["Britain, France, Russia", "Western & Eastern Europe", "the Allied Powers"],
...
json.dump(content, open(f"{OUT}/unit3_content.json", "w"), ensure_ascii=False, indent=1)
json.dump(exit_tickets, open(f"{OUT}/unit3_exit_tickets.json", "w"), ensure_ascii=False, indent=1)
# NOTE: unit3_images.json is produced by build_unit2_images.py — do NOT clobber it here.
