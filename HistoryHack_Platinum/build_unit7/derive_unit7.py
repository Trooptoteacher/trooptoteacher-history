#!/usr/bin/env python3
"""Derive analysis/unit7_content.json (+ exit tickets + images) for the Unit 2
...
OUT = "/home/user/Unit7_Claude_Core/analysis"

ican = json.load(open(f"{WEB}/ican/unit-7.json"))
vocab = json.load(open(f"{WEB}/vocabulary/unit-7.json"))
psrc = json.load(open(f"{WEB}/primary-sources/unit-7.json"))
dok = {}
for lvl in (1, 2, 3, 4):
    raw = json.load(open(f"{WEB}/questions/unit-7/dok-{lvl}.json"))
...
try:
    _factcards = json.load(open(f"{WEB}/factcards/unit-7.json"))
except Exception:
    _factcards = []
FC_BY_STD = {}
...
for idx, code in enumerate(ORDER):
    ...
    GEO_TASKS = {
        "US.62": "The Korean War was a Cold War fight over a divided country. Use the list below to map the key places, then explain how the 38th parallel shaped the war.",
        "US.66": "Vietnam was another divided-country Cold War war. Use the list below to map the key places, then explain the North-South split and the role of the Ho Chi Minh Trail.",
        "US.65": "The 1962 Cuban Missile Crisis was about GEOGRAPHY — how close is too close? Use the list below to map it, then explain why Soviet missiles in Cuba felt so dangerous.",
    }
    GEO_PLACES = {
        "US.62": [
            ["The 38th parallel", "Middle of Korea", "the Cold War dividing line between North and South"],
            ["North Korea", "Northern Korea", "communist, backed by the USSR and China"],
            ["South Korea", "Southern Korea", "backed by the U.S. and the United Nations"],
            ["Pusan", "Southeast corner", "the perimeter UN forces held early in the war"],
            ["Inchon", "West coast, near Seoul", "MacArthur's 1950 amphibious landing"],
        ],
        "US.66": [
            ["The 17th parallel", "Middle of Vietnam", "divided communist North from U.S.-backed South"],
            ["North Vietnam", "Northern Vietnam", "communist, led by Ho Chi Minh"],
            ["South Vietnam", "Southern Vietnam", "U.S.-backed government the U.S. fought to defend"],
            ["Ho Chi Minh Trail", "Laos & Cambodia border", "North's supply route into the South"],
            ["Gulf of Tonkin", "Off North Vietnam's coast", "site of the 1964 incident that widened the war"],
        ],
        "US.65": [
            ["Cuba", "~90 miles south of Florida", "Castro's communist revolution put a Soviet ally on America's doorstep"],
            ["Bay of Pigs", "Southern coast of Cuba", "failed 1961 U.S.-backed invasion by Cuban exiles"],
            ["Florida", "Southeastern U.S.", "within range of the missiles placed in Cuba"],
            ["The Soviet Union", "Eastern Europe / Asia", "secretly shipped nuclear missiles to Cuba in 1962"],
            ["Turkey", "Near the USSR's border", "site of U.S. missiles quietly removed to end the crisis"],
        ],
    }
    geo = GEO_TASKS.get(code, "")
    geo_places = GEO_PLACES.get(code, [])  # curated only — no generic G-tag fallback (avoids unscaffolded geo pages)
...
json.dump(content, open(f"{OUT}/unit7_content.json", "w"), ensure_ascii=False, indent=1)
json.dump(exit_tickets, open(f"{OUT}/unit7_exit_tickets.json", "w"), ensure_ascii=False, indent=1)
# NOTE: unit7_images.json is produced by build_unit2_images.py — do NOT clobber it here.
