#!/usr/bin/env python3
"""Derive analysis/unit7_assessment.json (Formative + Summative Form A/B) for the
...
ican = {s["standardCode"]: s for s in json.load(open(f"{WEB}/ican/unit-7.json"))}
ORDER = list(ican.keys())
dok = {}
for l in (1, 2, 3, 4):
    raw = json.load(open(f"{WEB}/questions/unit-7/dok-{l}.json"))
...
DISTINCT = {
 "US.59": ["cold war", "soviet union", "superpower", "arms race", "iron curtain", "communism", "ideolog"],
 "US.60": ["containment", "truman doctrine", "marshall plan", "nato", "warsaw pact", "kennan", "berlin airlift"],
 "US.61": ["mccarthy", "huac", "red scare", "blacklist", "hollywood ten", "rosenberg", "loyalty"],
 "US.62": ["korean war", "38th parallel", "macarthur", "inchon", "pusan", "armistice", "panmunjom"],
 "US.63": ["eisenhower", "new look", "massive retaliation", "domino theory", "brinkmanship", "dulles", "u-2"],
 "US.64": ["hydrogen bomb", "fallout shelter", "duck and cover", "civil defense", "arms race", "mutually assured"],
 "US.65": ["castro", "bay of pigs", "cuban missile crisis", "khrushchev", "blockade", "cuba"],
 "US.66": ["vietnam", "ho chi minh", "viet cong", "gulf of tonkin", "tet offensive", "domino"],
 "US.67": ["vietnamization", "escalation", "tonkin resolution", "nixon", "johnson", "advisers"],
 "US.68": ["anti-war", "kent state", "draft", "counterculture", "my lai", "silent majority", "pentagon papers", "protest"],
 "US.69": ["sputnik", "nasa", "apollo", "moon landing", "armstrong", "gagarin", "satellite", "space race"],
 "US.70": ["detente", "salt", "berlin wall", "gorbachev", "glasnost", "perestroika", "reagan"],
}
...
out = {"unit": "Unit 7", "formative": formative, "formA": formA, "formB": formB,
...
json.dump(out, open(f"{OUT}/unit7_assessment.json", "w"), ensure_ascii=False, indent=1)
