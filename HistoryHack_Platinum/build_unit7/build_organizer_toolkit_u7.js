...
const C=JSON.parse(fs.readFileSync('analysis/unit7_content.json','utf8')).standards;
...
// ---------- ASSEMBLE ----------
const header=new Header({children:[P(R('U.S. History Hack™ · Unit 7 · Teacher Graphic Organizer Toolkit',{s:16,b:true,c:GOLD}),{align:AlignmentType.RIGHT,spacing:{after:0}})]});
const footer=new Footer({children:[new Paragraph({alignment:AlignmentType.CENTER,spacing:{before:0},children:[R('U.S. History Hack™ · Unit 7 (Course Standard) © 2026 TroopToTeacher Technologies LLC   |   Page ',{s:16,c:GREY}),new TextRun({children:[PageNumber.CURRENT],size:16,color:GREY,font:FONT})]})]});
...
const cover=[
 ...
 new Paragraph({spacing:{after:200},children:[R('Unit 7 — The Cold War (1945–1991)  ·  Reproducible',{s:26,c:INK})]}),
...
Packer.toBuffer(doc).then(b=>{fs.writeFileSync('deliverables/Unit7_Teacher_Graphic_Organizer_Toolkit.docx',b);console.log('WROTE toolkit',b.length,'bytes');});
