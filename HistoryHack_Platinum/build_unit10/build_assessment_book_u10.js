// Unit 10 — TEACHER Assessment Book (Formative + Summative Form A/B + Teacher Key/Analysis).
...
const A=JSON.parse(fs.readFileSync('analysis/unit10_assessment.json','utf8'));
const C=JSON.parse(fs.readFileSync('analysis/unit10_content.json','utf8')).standards;
...
// ---- COVER + front ----
const header=new Header({children:[P(R('U.S. History Hack™ · Unit 10 · Assessment Book (Teacher)',{s:16,b:true,c:GOLD}),{align:AlignmentType.RIGHT,spacing:{after:0}})]});
const footer=new Footer({children:[new Paragraph({alignment:AlignmentType.CENTER,children:[R('U.S. History Hack™ · Unit 10 (Course Standard) © 2026 TroopToTeacher Technologies LLC   |   Page ',{s:16,c:GREY}),new TextRun({children:[PageNumber.CURRENT],size:16,color:GREY,font:FONT})]})]});
const cover=[
 ...
 new Paragraph({spacing:{after:200},children:[R('Unit 10 — Modern America (1965–2016)  ·  Teacher Edition',{s:26})]}),
...
Packer.toBuffer(doc).then(b=>{fs.writeFileSync('deliverables/Unit10_Assessment_Book_Teacher.docx',b);console.log('WROTE assessment book',b.length,'bytes');});
