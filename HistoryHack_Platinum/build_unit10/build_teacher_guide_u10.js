// Unit 10 Teacher How-to-Use & MTSS Guide — docx-js, EXACT Unit 5 template tokens/structure.
...
const body=[
  H('Unit 10 — Teacher How-to-Use & MTSS Guide',1),
  P(Rr('Modern America (1965–2016) · Course Standard · US.83–US.95 · Grades 9–12',{s:22,b:true,c:NAVY})),
  ...
  dataTable(['Criterion','Where it shows'],[
    ['Standards alignment (US.83–US.95)','Standards & SSP Crosswalk; per-standard blocks.'],
  ...
  callout('De-biased key distribution (Transfer Checks, US.83–US.95)',['A · B · C · D · A']),
  ...
  H('11. How CAST UDL 3.0 Is Visible in Unit 10',1),
...
const header=new Header({children:[P(Rr('U.S. History Hack™ · Unit 10 Teacher Guide',{s:16,b:true,c:GOLD}),{align:AlignmentType.RIGHT,spacing:{after:0}})]});
const footer=new Footer({children:[new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:0},children:[
  Rr('U.S. History Hack™ · Unit 10 Teacher Guide   © 2026 TroopToTeacher Technologies LLC   |   Page ',{s:15,c:GREY}),
  new TextRun({children:[PageNumber.CURRENT],size:15,color:GREY,bold:true,font:FONT})]})]});
const doc=new Document({creator:'TroopToTeacher Technologies LLC',title:'U.S. History Hack — Unit 10 Teacher How-to-Use & MTSS Guide (Pilot)',
...
Packer.toBuffer(doc).then(b=>{fs.writeFileSync('deliverables/Unit10_Teacher_HowToUse_and_MTSS_Guide.docx',b);console.log('WROTE teacher guide',b.length,'bytes');});
