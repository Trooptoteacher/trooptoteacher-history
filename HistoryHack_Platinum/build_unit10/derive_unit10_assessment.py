#!/usr/bin/env python3
"""Derive analysis/unit10_assessment.json (Formative + Summative Form A/B) for the
...
ican = {s["standardCode"]: s for s in json.load(open(f"{WEB}/ican/unit-10.json"))}
ORDER = list(ican.keys())
dok = {}
for l in (1, 2, 3, 4):
    raw = json.load(open(f"{WEB}/questions/unit-10/dok-{l}.json"))
...
DISTINCT = {
 "US.83": ["great society", "war on poverty", "medicare", "medicaid", "head start", "lyndon johnson", "lbj", "immigration act of 1965", "elementary and secondary education"],
 "US.84": ["counterculture", "hippie", "woodstock", "anti-war", "antiwar", "students for a democratic", "free speech movement", "kent state", "protest movement"],
 "US.85": ["nixon", "détente", "detente", "opening to china", "vietnamization", "silent majority", "revenue sharing", "salt", "environmental protection agency"],
 "US.86": ["watergate", "impeach", "resign", "cover-up", "cover up", "saturday night massacre", "executive privilege", "white house tapes", "articles of impeachment"],
 "US.87": ["environmental", "silent spring", "rachel carson", "earth day", "clean air act", "clean water act", "endangered species", "pollution", "conservation"],
 "US.88": ["carter", "camp david", "iran hostage", "hostage crisis", "panama canal", "stagflation", "energy crisis", "malaise", "sadat", "begin"],
 "US.89": ["reagan", "reaganomics", "supply-side", "supply side", "strategic defense", "star wars", "iran-contra", "iran contra", "tear down this wall", "sandinista"],
 "US.90": ["george h.w. bush", "persian gulf", "gulf war", "desert storm", "kuwait", "saddam", "berlin wall fell", "soviet union collapse", "americans with disabilities", "ada"],
 "US.91": ["clinton", "nafta", "welfare reform", "lewinsky", "balanced budget", "oklahoma city", "don't ask", "family and medical leave", "north american free trade"],
 "US.92": ["september 11", "9/11", "al-qaeda", "al qaeda", "twin towers", "world trade center", "patriot act", "homeland security", "war on terror", "osama", "taliban"],
 "US.93": ["title ix", "roe v. wade", "affirmative action", "glass ceiling", "sandra day o'connor", "first woman", "first african american", "immigration reform", "diversity"],
 "US.94": ["internet", "world wide web", "personal computer", "cell phone", "social media", "digital", "information age", "dot-com", "smartphone", "email"],
 "US.95": ["obama", "affordable care act", "obamacare", "first african american president", "great recession", "stimulus", "bin laden", "same-sex marriage", "paris climate"],
}
...
out = {"unit": "Unit 9", "formative": formative, "formA": formA, "formB": formB,
...
json.dump(out, open(f"{OUT}/unit10_assessment.json", "w"), ensure_ascii=False, indent=1)
