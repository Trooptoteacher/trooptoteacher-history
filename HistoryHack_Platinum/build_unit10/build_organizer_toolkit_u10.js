...
// ---------- ASSEMBLE ----------
const header=new Header({children:[P(R('U.S. History Hack™ · Unit 10 · Teacher Graphic Organizer Toolkit',{s:16,b:true,c:GOLD}),{align:AlignmentType.RIGHT,spacing:{after:0}})]});
const footer=new Footer({children:[new Paragraph({alignment:AlignmentType.CENTER,spacing:{before:0},children:[R('U.S. History Hack™ · Unit 10 (Course Standard) © 2026 TroopToTeacher Technologies LLC   |   Page ',{s:16,c:GREY}),new TextRun({children:[PageNumber.CURRENT],size:16,color:GREY,font:FONT})]})]});
...
const cover=[
 ...
 new Paragraph({spacing:{after:200},children:[R('Unit 10 — Modern America (1965–2016)  ·  Reproducible',{s:26,c:INK})]}),
...
Packer.toBuffer(doc).then(b=>{fs.writeFileSync('deliverables/Unit10_Teacher_Graphic_Organizer_Toolkit.docx',b);console.log('WROTE toolkit',b.length,'bytes');});
