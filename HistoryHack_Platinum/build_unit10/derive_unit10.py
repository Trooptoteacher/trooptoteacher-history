#!/usr/bin/env python3
"""Derive analysis/unit10_content.json (+ exit tickets + images) for the Unit 2
...
OUT = "/home/user/Unit10_Claude_Core/analysis"

ican = json.load(open(f"{WEB}/ican/unit-10.json"))
vocab = json.load(open(f"{WEB}/vocabulary/unit-10.json"))
psrc = json.load(open(f"{WEB}/primary-sources/unit-10.json"))
dok = {}
for lvl in (1, 2, 3, 4):
    raw = json.load(open(f"{WEB}/questions/unit-10/dok-{lvl}.json"))
...
content = {
    "unit": {
        "code": "Unit 9",
        "title": "The Civil Rights Movement (1954–1975)",
        "standards_range": "US.78–US.82",
...
for idx, code in enumerate(ORDER):
    ...
    # Unit 10 (Modern Era) is mostly domestic policy, but two standards carry a
    # genuine spatial dimension: the end of the Cold War / Persian Gulf War under
    # George H.W. Bush (US.90, tagged G in the standard) and the September 11th
    # attacks (US.92). Only these get a Geographer's Lens — no generic filler.
    GEO_TASKS = {
        "US.90": "The Cold War ended on the map, not just in speeches. As the Soviet bloc collapsed and the U.S. led a coalition in the Persian Gulf, geography decided who was freed, who was invaded, and where American power went next. Locate each place, then explain how its geography shaped the event.",
        "US.92": "The September 11, 2001 attacks struck three separate places on one morning — each chosen for what it symbolized and where it stood. Locate the three sites, then explain how geography shaped both the attacks and the nation's response.",
    }
    GEO_PLACES = {
        "US.90": [
            ["Berlin, Germany", "Central Europe", "Nov. 1989 — the Berlin Wall falls; the divided city reunites as the Cold War thaws"],
            ["Eastern Europe (Poland, Hungary, Czechoslovakia)", "Soviet bloc", "1989 — mostly peaceful revolutions end communist rule across the region"],
            ["Soviet Union → Russia", "Eurasia", "1991 — the USSR dissolves into 15 independent republics; the Cold War ends"],
            ["Kuwait & Iraq", "Persian Gulf", "1990–91 — Iraq invades Kuwait; a U.S.-led coalition drives it out in the Gulf War"],
            ["Saudi Arabia", "Persian Gulf", "Staging ground for Operation Desert Shield/Storm; oil geography shapes the coalition"],
        ],
        "US.92": [
            ["World Trade Center, New York City", "Northeast — financial capital", "Two hijacked jets destroy the Twin Towers, symbol of U.S. economic power"],
            ["The Pentagon, Arlington, Virginia", "Washington, D.C. area", "A third jet strikes the headquarters of the U.S. military"],
            ["Shanksville, Pennsylvania", "Rural Appalachia", "United 93 crashes after passengers fight back, short of its Washington target"],
        ],
    }
...
json.dump(content, open(f"{OUT}/unit10_content.json", "w"), ensure_ascii=False, indent=1)
json.dump(exit_tickets, open(f"{OUT}/unit10_exit_tickets.json", "w"), ensure_ascii=False, indent=1)
# NOTE: unit9_images.json is produced by build_unit2_images.py — do NOT clobber it here.
