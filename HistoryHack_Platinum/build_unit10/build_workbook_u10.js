// Unit 10 Course Standard STUDENT WORKBOOK — docx-js, EXACT Unit 5 template (no deviation).
...
const C=JSON.parse(fs.readFileSync('analysis/unit10_content.json','utf8'));
const IMG=fs.existsSync('analysis/unit10_images.json')?JSON.parse(fs.readFileSync('analysis/unit10_images.json','utf8')):{};
const EXIT=fs.existsSync('analysis/unit10_exit_tickets.json')?JSON.parse(fs.readFileSync('analysis/unit10_exit_tickets.json','utf8')):{};
...
const HERO={file:'analysis/assets/hero.jpg', w:1280, h:1010,
  cap:'President Ronald Reagan at the Brandenburg Gate, West Berlin — “Mr. Gorbachev, tear down this wall!” June 12, 1987.',
  cred:'Ronald Reagan Presidential Library / U.S. National Archives. Public domain.'};
function coverHero(rec,{maxW=470,maxH=360}={}){
  let w=Math.min(rec.w,maxW), h=Math.round(rec.h*(w/rec.w));
  if(h>maxH){ h=maxH; w=Math.round(rec.w*(h/rec.h)); }
  const out=[new Paragraph({alignment:AlignmentType.CENTER,spacing:{before:60,after:40},
  ...
  out.push(P(R(rec.cap,{s:16,i:true,c:GREY}),{align:AlignmentType.CENTER,spacing:{after:10}}));
  out.push(P(R('Source: '+rec.cred,{s:14,c:GREY}),{align:AlignmentType.CENTER,spacing:{after:120}}));
  return out;
}
...
// ================= COVER =================
const cover=[
  P(R("1776 - 2026   •   AMERICA'S 250TH ANNIVERSARY   •   SEMIQUINCENTENNIAL EDITION",{s:20,b:true,c:GOLD}),{align:AlignmentType.CENTER,spacing:{before:120,after:150}}),
  P(R('U.S. HISTORY HACK™',{s:44,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:40}}),
  P(R('TROOPTOTEACHER TECHNOLOGIES',{s:20,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:200}}),
  P(R('UNIT 10',{s:52,b:true,c:RED}),{align:AlignmentType.CENTER,spacing:{after:60}}),
  P(R('Modern America, 1965–2016',{s:28,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:100}}),
  P(R('COURSE STANDARD EDITION',{s:26,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:50}}),
  P(R('US.83 – US.95   •   13 Tennessee U.S. History Standards   •   Grades 9–12',{s:22,c:INK}),{align:AlignmentType.CENTER,spacing:{after:140}}),
  ...coverHero(HERO),
  callout('TENNESSEE CONNECTION · BAKER, GORE & GIG CITY',['Tennesseans shaped the modern era. Senator Howard Baker of Tennessee asked the defining Watergate question — “What did the President know, and when did he know it?” — then served as Ronald Reagan’s Chief of Staff (US.86, US.89). Al Gore of Carthage championed the “information superhighway” as Vice President and won the Nobel Peace Prize for climate advocacy (US.91, US.94). Fort Campbell’s 101st Airborne deployed to the Persian Gulf and, after 9/11, to Afghanistan (US.90, US.92). In 2010, Chattanooga — “Gig City” — launched the nation’s first citywide gigabit internet (US.94).']),
  P(R('One common workbook designed for learner variability. Every student works toward the same Tennessee standards; supports vary the means, not the goal or the ceiling.',{s:22,i:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{before:160,after:110}}),
  P([R('Release-Ready · Pilot Edition',{s:22,b:true,c:NAVY}),R('      © 2026 TroopToTeacher Technologies LLC · ISBN: [to be assigned]',{s:16,c:GREY})],{align:AlignmentType.CENTER,spacing:{after:0}}),
  PB()];

// ================= FRONT MATTER =================
const front=[
  H('Copyright, Ownership & Framework',1),
  P(R('U.S. History Hack™ — Unit 10: Modern America (1965–2016), Course Standard Edition.',{s:22,b:true})),
  ...
  P(R('Framework stack. This workbook anchors to Tennessee U.S. History standards US.83–US.95 and the Social Studies Practices (SSP.01–SSP.06), and is designed on CAST UDL 3.0 and an MTSS support model.',{s:22})),
...
function block(code){
  ...
  out.push(callout('PREVIEW & PREDICT',['Which learning target will be hardest, and why? Connect it to the unit’s big question: how did people change the law and the nation, and what did it cost?']));
  out.push(...ruled(s.tn_connection?2:3));
  ...
  const _cl=(a.close||'').length; const _long=_cl>1500; const _pf=_cl>2100?20:21;
  const _chunks=[];
  ...
  // NO-BLEED: cap rows so passage + Evidence Lab share ONE page. Long passages
  // (which eat vertical space) show 3 questions; shorter ones show up to 4.
  const _tdq=_long?a.tdq.slice(0,3):a.tdq.slice(0,4);
  out.push(writeTable(['Text-dependent question','Evidence from the passage','Your answer (what it shows)'],_tdq.map(q=>[q,'','']),[3248,2900,3500],{lines:2}));
  ...
  out.push(callout('STRETCH (EXTENSION — open to all)',['Think like a test-writer: jot one DOK-3 question of your own for this standard (see “Think Like a Test-Writer” at the end of the unit for the how-to).']));
  out.push(...ruled(2));
  // Activity 7 — CER
  ...
  out.push(callout('BIG-QUESTION ORGANIZER — how did Americans fight to make the promise of equality real, and what did it take to change the law and the nation?',['Jot quick notes from THIS standard: Who benefited? · Who bore the costs? · Who decided?']));
  out.push(...ruled(2));
...
const back=[
  ...
  callout('TENNESSEE CONNECTION · TENNESSEANS IN THE MODERN ERA',['Tennessee shaped the modern era at every turn. Senator Howard Baker of Tennessee asked the defining Watergate question — “What did the President know, and when did he know it?” — as vice chairman of the Senate Watergate Committee, then served as Ronald Reagan’s Chief of Staff (US.85, US.86, US.89). The Great Society reached deep into the state: the Appalachian Regional Commission rebuilt East Tennessee infrastructure, and Medicare and Head Start expanded statewide (US.83). The Tellico Dam fight over the snail darter produced the landmark TVA v. Hill environmental ruling (US.87). Al Gore of Carthage championed the “information superhighway” and climate action (US.91, US.94); Fort Campbell’s 101st Airborne deployed to the Gulf War and to Afghanistan after 9/11 (US.90, US.92); and in 2010 Chattanooga — “Gig City” — launched the nation’s first citywide gigabit internet (US.94).']),
  ...
  P(R('The modern era reached every Tennessee community — through federal programs, new technology, military service, and changing industries. Investigate one local connection and record what you find.',{s:22})),
  ...
  H('The Making of Modern America — Four Threads',1),
  P(R('This section connects the threads of the era: between the 1960s and today, Americans remade the role of government, weathered crises of trust and economy, ended the Cold War, and confronted a new century. It is History Hack-authored instructional synthesis grounded in this unit’s sourced record — not a primary source.',{s:22})),
  ...[['A New Domestic Order','Johnson’s Great Society vastly expanded the federal safety net — Medicare, Medicaid, the War on Poverty, and federal aid to schools (US.83) — while a youth counterculture and new activist movements challenged authority and widened the meaning of rights (US.84).'],
    ['A Crisis of Trust','Vietnam and then the Watergate scandal shattered public faith in government: Nixon’s achievements abroad were eclipsed by a cover-up that forced his resignation (US.85–US.86), and Carter inherited stagflation, an energy crisis, and the Iran hostage crisis (US.88).'],
    ['Conservative Resurgence & the Cold War’s End','Reagan cut taxes, expanded defense, and pressed the Soviet Union (US.89); under Reagan and George H.W. Bush the Berlin Wall fell and the USSR dissolved, and a U.S.-led coalition won the Gulf War (US.90). Clinton then governed a booming, globalizing, increasingly digital economy (US.91, US.94).'],
    ['A New Century’s Challenges','The September 11 attacks launched a long war on terror and reshaped security at home (US.92); a more diverse nation saw women and minorities rise to the highest offices, culminating in Obama’s presidency (US.93, US.95); and environmental awareness grew throughout the era (US.87).']
  ].flatMap(([h,b])=>[H(h,2),P(R(b,{s:21}))]),
  H('Unit Reflection',2),
  P(R('Return to the unit’s big question: how did Americans remake government, the economy, and the nation’s place in the world between the 1960s and today — and what did it cost? Write a final claim with two pieces of evidence from different standards.',{s:22})),
...
// ================= ASSEMBLE =================
const header=new Header({children:[P(R('U.S. History Hack™ · Unit 10 · Course Standard Edition',{s:16,b:true,c:GOLD}),{align:AlignmentType.RIGHT,spacing:{after:0}})]});
const footer=new Footer({children:[new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:0},children:[
  R('U.S. History Hack™ · Unit 10 (Course Standard)   © 2026 TroopToTeacher Technologies LLC   |   Page ',{s:15,c:GREY}),
  new TextRun({children:[PageNumber.CURRENT],size:15,color:GREY,bold:true,font:FONT})]})]});
const doc=new Document({creator:'TroopToTeacher Technologies LLC',title:'U.S. History Hack — Unit 10 Course Standard Student Workbook (Pilot)',
  ...
  sections:[{properties:{page:{size:{width:12240,height:15840},margin:{top:1152,bottom:1152,left:1296,right:1296,header:720,footer:720}}},
    headers:{default:header},footers:{default:footer},children: SAMPLE ? [P(R(`SAMPLE — Unit 10 · first ${SAMPLE} standards (activities only, ruled-line writing) — for review`,{s:24,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:120}}), ...standards] : [...cover,...front,...standards,...back]}]});
const OUTFILE=SAMPLE?`deliverables/Unit10_SAMPLE_${SAMPLE}standards.docx`:'deliverables/Unit10_Student_Workbook_CourseStandard.docx';
