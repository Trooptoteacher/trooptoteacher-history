// Unit 9 Course Standard STUDENT WORKBOOK — docx-js, EXACT Unit 5 template (no deviation).
...
const IMG=fs.existsSync('analysis/unit9_images.json')?JSON.parse(fs.readFileSync('analysis/unit9_images.json','utf8')):{};
...
const HERO={file:'analysis/assets/hero.jpg', w:588, h:458,
  cap:'The March on Washington for Jobs and Freedom, August 28, 1963.',
  cred:'U.S. National Archives. Public domain.'};
...
const cover=[
  ...
  P(R('UNIT 9',{s:52,b:true,c:RED}),{align:AlignmentType.CENTER,spacing:{after:60}}),
  P(R('The Civil Rights Movement, 1954–1975',{s:28,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:120}}),
  P(R('COURSE STANDARD EDITION',{s:26,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:60}}),
  P(R('US.78 – US.82   •   5 Tennessee U.S. History Standards   •   Grades 9–12',{s:22,c:INK}),{align:AlignmentType.CENTER,spacing:{after:140}}),
  ...coverHero(HERO),
  callout('TENNESSEE CONNECTION · NASHVILLE, HIGHLANDER & MEMPHIS',['Tennessee was at the heart of the civil rights movement. The Highlander Folk School in Monteagle trained leaders including Rosa Parks and Dr. Martin Luther King Jr. (US.79). In 1960, Fisk University students — among them Diane Nash and John Lewis — led the disciplined Nashville sit-ins that desegregated the city’s lunch counters (US.80). And in 1968, Dr. King came to Memphis to support striking sanitation workers, where he was assassinated at the Lorraine Motel (US.80).']),
...
const front=[
  ...
  P(R('U.S. History Hack™ — Unit 9: The Civil Rights Movement (1954–1975), Course Standard Edition.',{s:22,b:true})),
  ...
  P(R('Framework stack. This workbook anchors to Tennessee U.S. History standards US.78–US.82 and the Social Studies Practices (SSP.01–SSP.06), and is designed on CAST UDL 3.0 and an MTSS support model.',{s:22})),
...
function block(code){
  ...
  {const _wl=s.vocab.reduce((a,v)=>a+Math.max(1,Math.ceil((v.def||'').length/52)),0);
   ...
   if(_sparse){
     ...
     const _slots=s.vocab.length===1?3:s.vocab.length;   // total write blocks on the page
     const per=Math.max(2,Math.min(6,Math.round(Math.max(3,18-_wl)/_slots)));  // adaptive to definition length
...
const back=[
  ...
  callout('TENNESSEE CONNECTION · TENNESSEE AT THE HEART OF THE MOVEMENT',['Few states were more central to the civil rights movement than Tennessee. In the mountains at Monteagle, the Highlander Folk School trained a generation of activists — Rosa Parks attended just months before the Montgomery bus boycott, and Dr. Martin Luther King Jr. spoke there (US.79, US.82). In Nashville, Fisk and other students led by Diane Nash and John Lewis ran the 1960 lunch-counter sit-ins with strict nonviolent discipline, making the city a national model and launching leaders of the wider movement (US.80). Tennessee schools desegregated unevenly after Brown, with Clinton among the first (US.78). And in April 1968, Dr. King traveled to Memphis to stand with striking sanitation workers — “I AM A MAN” — where he delivered his final “Mountaintop” speech and was assassinated at the Lorraine Motel (US.80–US.81).']),
  ...
  H('Making Equality Real — Many Movements, Many Methods',1),
  P(R('This section connects the threads of the era: the fight to make the promise of equality real took many movements and many methods — courts, marches, laws, and grassroots organizing. It is History Hack-authored instructional synthesis grounded in this unit’s sourced record — not a primary source.',{s:22})),
  ...[['Winning in the Courts','The NAACP’s long legal campaign overturned “separate but equal” in Brown v. Board of Education (US.78), declaring segregated schools unconstitutional and giving the movement a legal foundation — though enforcement was slow and fiercely resisted.'],
    ['Winning in the Streets','Ordinary people forced change through nonviolent direct action (US.79–US.80): the Montgomery bus boycott, the Nashville and Greensboro sit-ins, the Freedom Rides, Birmingham, and the March on Washington built pressure the nation could not ignore.'],
    ['Winning in the Law','That pressure produced landmark legislation (US.81): the Civil Rights Act of 1964 outlawed segregation and job discrimination, and the Voting Rights Act of 1965 struck down the barriers that had denied Black Southerners the ballot.'],
    ['A Wider Struggle','The movement inspired others (US.82): the American Indian Movement, the Chicano movement and the United Farm Workers, and a revived women’s movement all pressed the country to widen the meaning of equality.']
  ...
  H('Unit Reflection',2),
  P(R('Return to the unit’s big question: how did Americans fight to make the promise of equality real — and what did it take to change the law and the nation? Write a final claim with two pieces of evidence from different standards.',{s:22})),
...
// ================= ASSEMBLE =================
const header=new Header({children:[P(R('U.S. History Hack™ · Unit 9 · Course Standard Edition',{s:16,b:true,c:GOLD}),{align:AlignmentType.RIGHT,spacing:{after:0}})]});
const footer=new Footer({children:[new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:0},children:[
  R('U.S. History Hack™ · Unit 9 (Course Standard)   © 2026 TroopToTeacher Technologies LLC   |   Page ',{s:15,c:GREY}),
  new TextRun({children:[PageNumber.CURRENT],size:15,color:GREY,bold:true,font:FONT})]})]});
const doc=new Document({creator:'TroopToTeacher Technologies LLC',title:'U.S. History Hack — Unit 9 Course Standard Student Workbook (Pilot)',
  ...
  sections:[{properties:{page:{size:{width:12240,height:15840},margin:{top:1152,bottom:1152,left:1296,right:1296,header:720,footer:720}}},
    headers:{default:header},footers:{default:footer},children: SAMPLE ? [P(R(`SAMPLE — Unit 9 · first ${SAMPLE} standards (activities only, ruled-line writing) — for review`,{s:24,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:120}}), ...standards] : [...cover,...front,...standards,...back]}]});
const OUTFILE=SAMPLE?`deliverables/Unit9_SAMPLE_${SAMPLE}standards.docx`:'deliverables/Unit9_Student_Workbook_CourseStandard.docx';
