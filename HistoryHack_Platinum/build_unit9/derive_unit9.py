#!/usr/bin/env python3
"""Derive analysis/unit9_content.json (+ exit tickets + images) for the Unit 2
...
OUT = "/home/user/Unit9_Claude_Core/analysis"

ican = json.load(open(f"{WEB}/ican/unit-9.json"))
vocab = json.load(open(f"{WEB}/vocabulary/unit-9.json"))
psrc = json.load(open(f"{WEB}/primary-sources/unit-9.json"))
dok = {}
for lvl in (1, 2, 3, 4):
    raw = json.load(open(f"{WEB}/questions/unit-9/dok-{lvl}.json"))
...
try:
    _factcards = json.load(open(f"{WEB}/factcards/unit-9.json"))
except Exception:
    _factcards = []
FC_BY_STD = {}
...
for idx, code in enumerate(ORDER):
    ...
    GEO_TASKS = {
        "US.80": "The civil rights movement unfolded city by city across the South. Use the list below to map the key events, then explain why the movement targeted these particular places.",
    }
    GEO_PLACES = {
        "US.80": [
            ["Montgomery, Alabama", "Deep South", "1955–56 bus boycott sparked by Rosa Parks; MLK emerges"],
            ["Little Rock, Arkansas", "Upper South", "1957 — federal troops escort the 'Little Rock Nine' into Central High"],
            ["Nashville, Tennessee", "Tennessee", "1960 lunch-counter sit-ins led by Diane Nash and John Lewis"],
            ["Birmingham, Alabama", "Deep South", "1963 campaign; the children's march and police violence shock the nation"],
            ["Selma, Alabama", "Deep South", "1965 voting-rights march ('Bloody Sunday') to Montgomery"],
            ["Memphis, Tennessee", "Tennessee", "1968 sanitation strike; Dr. King assassinated at the Lorraine Motel"],
        ],
    }
...
json.dump(content, open(f"{OUT}/unit9_content.json", "w"), ensure_ascii=False, indent=1)
json.dump(exit_tickets, open(f"{OUT}/unit9_exit_tickets.json", "w"), ensure_ascii=False, indent=1)
