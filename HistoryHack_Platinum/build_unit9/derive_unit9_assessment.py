#!/usr/bin/env python3
"""Derive analysis/unit9_assessment.json (Formative + Summative Form A/B) for the
...
ican = {s["standardCode"]: s for s in json.load(open(f"{WEB}/ican/unit-9.json"))}
ORDER = list(ican.keys())
dok = {}
for l in (1, 2, 3, 4):
    raw = json.load(open(f"{WEB}/questions/unit-9/dok-{l}.json"))
...
DISTINCT = {
 "US.78": ["brown v. board", "separate but equal", "plessy", "thurgood marshall", "naacp", "desegregation", "topeka", "integration"],
 "US.79": ["martin luther king", "malcolm x", "nonviolence", "sclc", "sncc", "black panther", "massive resistance", "wallace", "advocate"],
 "US.80": ["montgomery bus boycott", "rosa parks", "little rock", "sit-in", "freedom rider", "birmingham", "march on washington", "selma", "nashville"],
 "US.81": ["civil rights act", "voting rights act", "1964", "1965", "24th amendment", "poll tax", "literacy test"],
 "US.82": ["american indian movement", "chicano", "cesar chavez", "united farm workers", "feminis", "national organization for women", "equal rights amendment", "friedan"],
}
...
json.dump(out, open(f"{OUT}/unit9_assessment.json", "w"), ensure_ascii=False, indent=1)
