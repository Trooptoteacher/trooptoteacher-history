#!/usr/bin/env python3
"""Derive analysis/unit5_content.json (+ exit tickets + images) for the Unit 2
...
OUT = "/home/user/Unit5_Claude_Core/analysis"

ican = json.load(open(f"{WEB}/ican/unit-5.json"))
vocab = json.load(open(f"{WEB}/vocabulary/unit-5.json"))
psrc = json.load(open(f"{WEB}/primary-sources/unit-5.json"))
dok = {}
for lvl in (1, 2, 3, 4):
    raw = json.load(open(f"{WEB}/questions/unit-5/dok-{lvl}.json"))
...
try:
    _factcards = json.load(open(f"{WEB}/factcards/unit-5.json"))
except Exception:
    _factcards = []
FC_BY_STD = {}
...
for idx, code in enumerate(ORDER):
    ...
    # The Dust Bowl (US.40) is the one Great-Depression standard with a real
    # spatial dimension — drought + poor farming practices devastated a specific
    # region and drove a mass migration. No generic filler elsewhere.
    GEO_TASKS = {
        "US.40": "The Dust Bowl was a geographic disaster: drought, wind, and over-plowed land turned the southern Great Plains to dust and drove hundreds of thousands west. Locate the region and the migration route, then explain how geography caused the crisis and shaped where people fled.",
    }
    GEO_PLACES = {
        "US.40": [
            ["Oklahoma Panhandle", "Southern Great Plains", "epicenter — drought and dust storms ruined farms; 'Okies' fled west"],
            ["Texas Panhandle & western Kansas", "Southern Great Plains", "over-plowed wheat land turned to blowing dust"],
            ["Eastern Colorado & New Mexico", "Great Plains", "outer edge of the Dust Bowl damage"],
            ["Route 66", "Plains → West", "the migration road migrants followed toward California"],
            ["California (Central Valley)", "West Coast", "destination — migrants sought farm work but met hardship and prejudice"],
        ],
    }
...
json.dump(content, open(f"{OUT}/unit5_content.json", "w"), ensure_ascii=False, indent=1)
json.dump(exit_tickets, open(f"{OUT}/unit5_exit_tickets.json", "w"), ensure_ascii=False, indent=1)
# NOTE: unit5_images.json is produced by build_unit2_images.py — do NOT clobber it here.
