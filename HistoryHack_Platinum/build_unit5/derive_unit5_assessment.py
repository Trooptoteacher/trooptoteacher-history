#!/usr/bin/env python3
"""Derive analysis/unit5_assessment.json (Formative + Summative Form A/B) for the
...
ican = {s["standardCode"]: s for s in json.load(open(f"{WEB}/ican/unit-5.json"))}
ORDER = list(ican.keys())
dok = {}
for l in (1, 2, 3, 4):
    raw = json.load(open(f"{WEB}/questions/unit-5/dok-{l}.json"))
...
DISTINCT = {
 "US.39": ["stock market crash", "black tuesday", "overproduction", "buying on margin", "speculation", "bank failure", "1929", "causes of the great depression", "uneven distribution"],
 "US.40": ["dust bowl", "drought", "okies", "grapes of wrath", "great plains", "dust storm", "black blizzard", "soil", "over-plow", "migrant farm"],
 "US.41": ["hooverville", "bread line", "soup kitchen", "unemployment", "bonus army", "breadline", "homeless", "poverty", "shantytown", "impact of the great depression"],
 "US.42": ["hoover", "rugged individualism", "reconstruction finance", "hawley-smoot", "smoot-hawley", "volunteer", "boulder dam", "hoover dam", "laissez-faire"],
 "US.43": ["new deal", "fdr", "franklin roosevelt", "fireside chat", "ccc", "wpa", "tva", "social security", "aaa", "hundred days", "alphabet agenc"],
 "US.44": ["court-packing", "supreme court", "critics of the new deal", "huey long", "deficit spending", "welfare state", "second new deal", "share our wealth", "constitutionality"],
}
...
json.dump(out, open(f"{OUT}/unit5_assessment.json", "w"), ensure_ascii=False, indent=1)
