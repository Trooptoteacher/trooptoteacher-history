// Unit 5 Course Standard STUDENT WORKBOOK — docx-js, EXACT Unit 5 template (no deviation).
...
const C=JSON.parse(fs.readFileSync('analysis/unit5_content.json','utf8'));
const IMG=fs.existsSync('analysis/unit5_images.json')?JSON.parse(fs.readFileSync('analysis/unit5_images.json','utf8')):{};
const EXIT=fs.existsSync('analysis/unit5_exit_tickets.json')?JSON.parse(fs.readFileSync('analysis/unit5_exit_tickets.json','utf8')):{};
...
const HERO={file:'analysis/assets/hero.jpg', w:800, h:1040,
  cap:'Dorothea Lange, “Migrant Mother” — a destitute pea-picker and her children, Nipomo, California, 1936.',
  cred:'Dorothea Lange, Farm Security Administration / Library of Congress. Public domain.'};
...
const cover=[
  ...
  P(R('UNIT 5',{s:52,b:true,c:RED}),{align:AlignmentType.CENTER,spacing:{after:60}}),
  P(R('The Great Depression & New Deal, 1929–1941',{s:30,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:120}}),
  P(R('COURSE STANDARD EDITION',{s:26,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:60}}),
  P(R('US.39 – US.44   •   6 Tennessee U.S. History Standards   •   Grades 9–12',{s:22,c:INK}),{align:AlignmentType.CENTER,spacing:{after:140}}),
  ...coverHero(HERO),
  callout('TENNESSEE CONNECTION · THE TENNESSEE VALLEY AUTHORITY',['No New Deal program changed Tennessee more than the Tennessee Valley Authority (US.43). Created in 1933, the TVA built a chain of dams along the Tennessee River — beginning with Norris Dam — to control floods, generate cheap electricity, and put thousands to work. For the first time it brought electric power to rural farms and reshaped the valley’s whole economy. Civilian Conservation Corps camps across the state planted forests and built parks (US.43), even as bank failures and unemployment hit Tennessee hard.']),
...
const front=[
  ...
  P(R('U.S. History Hack™ — Unit 5: The Great Depression & New Deal (1929–1941), Course Standard Edition.',{s:22,b:true})),
  ...
  P(R('Framework stack. This workbook anchors to Tennessee U.S. History standards US.39–US.44 and the Social Studies Practices (SSP.01–SSP.06), and is designed on CAST UDL 3.0 and an MTSS support model.',{s:22})),
...
function block(code){
  ...
  out.push(callout('PREVIEW & PREDICT',['Which learning target will be hardest, and why? Connect it to the unit’s big question: who shared in the boom, and who was left out or pushed back?']));
  ...
  out.push(callout('BIG-QUESTION ORGANIZER — how did the Great Depression change what Americans expected from their government, and who did the New Deal help or leave out?',['Jot quick notes from THIS standard: Who was hurt? · Who was helped? · Who decided?']));
  out.push(...ruled(2));
...
const back=[
  ...
  callout('TENNESSEE CONNECTION · THE TENNESSEE VALLEY AUTHORITY',['No New Deal program left a deeper mark on Tennessee than the Tennessee Valley Authority (US.43). Signed into law in 1933, the TVA built a chain of dams along the Tennessee River — starting with Norris Dam north of Knoxville — to tame the river’s floods, generate cheap public electricity, and put thousands of unemployed men to work. For the first time it wired rural Tennessee farms for electric power, remaking daily life and the valley’s economy. Civilian Conservation Corps camps across the state planted forests and built the parks Tennesseans still use (US.43). Yet the Depression’s hardships — failed banks, lost farms, and joblessness — hit Tennessee hard, and the TVA’s dams also flooded towns and displaced families, a reminder that even celebrated New Deal projects carried costs (US.44).']),
  ...
  P(R('The Great Depression and the New Deal reached every Tennessee community — through TVA dams and power lines, CCC and WPA jobs, bank failures, and displaced farmers. Investigate one local connection and record what you find.',{s:22})),
  ...
  H('The Great Depression & New Deal — Four Threads',1),
  P(R('This section connects the threads of the era: how a booming economy collapsed, what the hardship looked like, how two presidents responded, and how the New Deal remade the role of government. It is History Hack-authored instructional synthesis grounded in this unit’s sourced record — not a primary source.',{s:22})),
  ...[['The Collapse','Overproduction, uneven wealth, reckless stock speculation, and buying on margin left the economy fragile; the October 1929 crash and a wave of bank failures plunged the nation into depression (US.39). On the southern Plains, drought and over-plowed land created the Dust Bowl, ruining farms and driving families west (US.40).'],
    ['The Human Toll','Unemployment reached one in four workers. Hoovervilles, breadlines, and soup kitchens spread; the Bonus Army marched on Washington; and hunger and homelessness marked the depths of the crisis (US.41).'],
    ['Two Responses','Hoover trusted voluntary action and limited federal aid, and the Depression deepened (US.42). Franklin Roosevelt answered with the New Deal — relief, recovery, and reform through a flurry of “alphabet agencies” (CCC, WPA, TVA, AAA) and fireside chats that restored confidence (US.43).'],
    ['A Bigger Government — and Its Critics','The New Deal permanently expanded the federal government’s role and left lasting programs like Social Security. But it drew fire from critics on the left and right and clashed with the Supreme Court, raising debates about federal power that continue today (US.44).']
  ].flatMap(([h,b])=>[H(h,2),P(R(b,{s:21}))]),
  H('Unit Reflection',2),
  P(R('Return to the unit’s big question: how did the Great Depression change what Americans expected from their government — and who did the New Deal help or leave out? Write a final claim with two pieces of evidence from different standards.',{s:22})),
  writeTable(['Part','Write here'],[['My claim',''],['Evidence 1 (standard: ___)',''],['Evidence 2 (standard: ___)',''],['Why it matters today','']],[2600,7048],{rowH:820,noSplit:true})];

// ================= ASSEMBLE =================
const header=new Header({children:[P(R('U.S. History Hack™ · Unit 5 · Course Standard Edition',{s:16,b:true,c:GOLD}),{align:AlignmentType.RIGHT,spacing:{after:0}})]});
const footer=new Footer({children:[new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:0},children:[
  R('U.S. History Hack™ · Unit 5 (Course Standard)   © 2026 TroopToTeacher Technologies LLC   |   Page ',{s:15,c:GREY}),
  new TextRun({children:[PageNumber.CURRENT],size:15,color:GREY,bold:true,font:FONT})]})]});
const doc=new Document({creator:'TroopToTeacher Technologies LLC',title:'U.S. History Hack — Unit 5 Course Standard Student Workbook (Pilot)',
  ...
  sections:[{properties:{page:{size:{width:12240,height:15840},margin:{top:1152,bottom:1152,left:1296,right:1296,header:720,footer:720}}},
    headers:{default:header},footers:{default:footer},children: SAMPLE ? [P(R(`SAMPLE — Unit 5 · first ${SAMPLE} standards (activities only, ruled-line writing) — for review`,{s:24,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:120}}), ...standards] : [...cover,...front,...standards,...back]}]});
const OUTFILE=SAMPLE?`deliverables/Unit5_SAMPLE_${SAMPLE}standards.docx`:'deliverables/Unit5_Student_Workbook_CourseStandard.docx';
