#!/usr/bin/env python3
"""Derive analysis/unit2_content.json (+ exit tickets + images) for the Unit 2
...
ican = json.load(open(f"{WEB}/ican/unit-2.json"))
vocab = json.load(open(f"{WEB}/vocabulary/unit-2.json"))
psrc = json.load(open(f"{WEB}/primary-sources/unit-2.json"))
dok = {}
for lvl in (1, 2, 3, 4):
    raw = json.load(open(f"{WEB}/questions/unit-2/dok-{lvl}.json"))
...
try:
    _factcards = json.load(open(f"{WEB}/factcards/unit-2.json"))
except Exception:
    _factcards = []
FC_BY_STD = {}
...
for idx, code in enumerate(ORDER):
    ...
    GEO_TASKS = {
        "US.11": "Locate the major sites of labor unrest in this era and mark what workers there were fighting for. The places and where they are are listed below — you do NOT have to know them from memory.",
        "US.18": "Women’s suffrage spread unevenly across the country before the 19th Amendment passed in 1920. Use the regions below to map where the vote came early and where it came last — then explain the pattern.",
    }
    # Grounded place word bank (place + WHERE it is) so the mapping activity has
    # real support — students are never asked to draw a map from scratch.
    GEO_PLACES = {
        "US.11": [
            ["Homestead", "Pennsylvania", "1892 Homestead Strike at Carnegie’s steel works"],
            ["Pullman / Chicago", "Illinois", "1894 Pullman railcar strike"],
            ["Coal Creek / Anderson County", "Tennessee", "1891–92 Coal Creek War over convict-lease labor"],
        ],
        "US.18": [
            ["Western states (e.g., Wyoming, Colorado)", "The West", "granted women the vote earliest — Wyoming Territory in 1869"],
            ["Tennessee", "The South", "the 36th and final state needed — ratified August 18, 1920"],
            ["Most Southern states", "The South", "ratified last, or rejected the 19th Amendment"],
        ],
    }
...
json.dump(content, open(f"{OUT}/unit2_content.json", "w"), ensure_ascii=False, indent=1)
json.dump(exit_tickets, open(f"{OUT}/unit2_exit_tickets.json", "w"), ensure_ascii=False, indent=1)
# NOTE: unit2_images.json is produced by build_unit2_images.py — do NOT clobber it here.
