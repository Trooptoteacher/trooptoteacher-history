// Unit 2 Course Standard STUDENT WORKBOOK — docx-js, EXACT Unit 5 template (no deviation).
...
const C=JSON.parse(fs.readFileSync('analysis/unit2_content.json','utf8'));
const IMG=fs.existsSync('analysis/unit2_images.json')?JSON.parse(fs.readFileSync('analysis/unit2_images.json','utf8')):{};
const EXIT=fs.existsSync('analysis/unit2_exit_tickets.json')?JSON.parse(fs.readFileSync('analysis/unit2_exit_tickets.json','utf8')):{};
...
const HERO={file:'analysis/assets/hero.jpg', w:1536, h:1204,
  cap:'Crowds fill Pennsylvania Avenue in Washington, D.C., during the Progressive Era.',
  cred:'Library of Congress. Public domain.'};
...
const cover=[
  ...
  P(R('UNIT 2',{s:52,b:true,c:RED}),{align:AlignmentType.CENTER,spacing:{after:60}}),
  P(R('The Progressive Era, 1890–1920',{s:32,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:120}}),
  P(R('COURSE STANDARD EDITION',{s:26,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:60}}),
  P(R('US.08 – US.18   •   11 Tennessee U.S. History Standards   •   Grades 9–12',{s:22,c:INK}),{align:AlignmentType.CENTER,spacing:{after:140}}),
  ...coverHero(HERO),
  callout('TENNESSEE CONNECTION · THE PERFECT 36',['Tennessee cast the deciding vote for women’s suffrage: on August 18, 1920 it became the 36th and final state to ratify the 19th Amendment, when 24-year-old legislator Harry T. Burn changed his vote after a letter from his mother (US.18).']),
...
const front=[
  ...
  P(R('U.S. History Hack™ — Unit 2: The Progressive Era (1890–1920), Course Standard Edition.',{s:22,b:true})),
  ...
  P(R('Framework stack. This workbook anchors to Tennessee U.S. History standards US.08–US.18 and the Social Studies Practices (SSP.01–SSP.06), and is designed on CAST UDL 3.0 and an MTSS support model.',{s:22})),
...
function block(code){
  ...
  out.push(callout('PREVIEW & PREDICT',['Which learning target will be hardest, and why? Connect it to the unit’s big question: who benefited and who bore the costs?']));
  ...
  out.push(callout('BIG-QUESTION ORGANIZER — as the U.S. industrialized, who benefited and who bore the costs?',['Jot quick notes from THIS standard: Who benefited? · Who bore the costs? · Who decided?']));
  out.push(...ruled(2));
...
const back=[
  ...
  callout('TENNESSEE CONNECTION · THE PERFECT 36',['On August 18, 1920, Tennessee became the 36th and final state needed to ratify the 19th Amendment, giving women the vote nationwide. The tie in the state House was broken by 24-year-old legislator Harry T. Burn, who changed his vote after a letter from his mother urged him to “be a good boy” and help Mrs. Catt. Tennessee’s decision (US.18) turned a Progressive-era movement into the law of the land — the last state the whole country was waiting on.']),
  ...
  H('The Progressive Era — Reached and Left Out',1),
  P(R('This section complicates the Progressive-era narrative: who reform reached, who it excluded, and who organized in response. It is History Hack-authored instructional synthesis grounded in this unit’s sourced record — not a primary source.',{s:22})),
  ...[['Industrial Workers','Long hours, low pay, and dangerous conditions drove the rise of unions and strikes (US.11); the Coal Creek War (1891–92) in Anderson County, Tennessee pitted miners against the convict-lease system.'],
    ['Women & Children','Women and children worked textile mills, mines, and factories under harsh conditions (US.13); Progressive reformers pushed the first child-labor and workplace-safety laws, and the suffrage movement won the vote in 1920 (US.18).'],
    ['African Americans','Booker T. Washington and W.E.B. Du Bois offered competing paths to advancement under Jim Crow (US.09); most Progressive reforms did little to challenge segregation, and Black Americans built their own institutions and the early civil-rights organizing that followed.'],
    ['Immigrants & the Urban Poor','Muckrakers exposed tenement poverty, unsafe food, and machine politics (US.14); reformers pressed for regulation of trusts and monopolies (US.12), while many immigrants were still met with nativism.']
  ...
  H('Unit Reflection',2),
  P(R('Return to the unit’s big question: as reformers tried to fix the problems of the industrial age, whose lives improved, who was left out, and how much did government change? Write a final claim with two pieces of evidence from different standards.',{s:22})),
...
// ================= ASSEMBLE =================
const header=new Header({children:[P(R('U.S. History Hack™ · Unit 2 · Course Standard Edition',{s:16,b:true,c:GOLD}),{align:AlignmentType.RIGHT,spacing:{after:0}})]});
const footer=new Footer({children:[new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:0},children:[
  R('U.S. History Hack™ · Unit 2 (Course Standard)   © 2026 TroopToTeacher Technologies LLC   |   Page ',{s:15,c:GREY}),
  new TextRun({children:[PageNumber.CURRENT],size:15,color:GREY,bold:true,font:FONT})]})]});
const doc=new Document({creator:'TroopToTeacher Technologies LLC',title:'U.S. History Hack — Unit 2 Course Standard Student Workbook (Pilot)',
  ...
  sections:[{properties:{page:{size:{width:12240,height:15840},margin:{top:1152,bottom:1152,left:1296,right:1296,header:720,footer:720}}},
    headers:{default:header},footers:{default:footer},children: SAMPLE ? [P(R(`SAMPLE — Unit 2 · first ${SAMPLE} standards (activities only, ruled-line writing) — for review`,{s:24,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:120}}), ...standards] : [...cover,...front,...standards,...back]}]});
const OUTFILE=SAMPLE?`deliverables/Unit2_SAMPLE_${SAMPLE}standards.docx`:'deliverables/Unit2_Student_Workbook_CourseStandard.docx';
