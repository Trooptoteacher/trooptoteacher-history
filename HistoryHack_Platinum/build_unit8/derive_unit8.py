#!/usr/bin/env python3
"""Derive analysis/unit8_content.json (+ exit tickets + images) for the Unit 2
...
OUT = "/home/user/Unit8_Claude_Core/analysis"

ican = json.load(open(f"{WEB}/ican/unit-8.json"))
vocab = json.load(open(f"{WEB}/vocabulary/unit-8.json"))
psrc = json.load(open(f"{WEB}/primary-sources/unit-8.json"))
dok = {}
for lvl in (1, 2, 3, 4):
    raw = json.load(open(f"{WEB}/questions/unit-8/dok-{lvl}.json"))
...
try:
    _factcards = json.load(open(f"{WEB}/factcards/unit-8.json"))
except Exception:
    _factcards = []
FC_BY_STD = {}
...
content = {
    "unit": {
        "code": "Unit 8",
        "title": "Postwar America (1945–1963)",
        "standards_range": "US.71–US.77",
...
for idx, code in enumerate(ORDER):
    ...
    GEO_TASKS = {
        "US.74": "The automobile and the new Interstate Highway System (1956) remade where and how Americans lived. Use the list below to map the change, then explain how highways created suburbs and pulled people out of city centers.",
    }
    GEO_PLACES = {
        "US.74": [
            ["The Interstate Highway System", "Nationwide", "1956 law — ~41,000 miles of new limited-access highways"],
            ["I-40 across Tennessee", "Memphis–Nashville–Knoxville", "east-west interstate linking the state's big cities"],
            ["New suburbs", "Outside Nashville, Memphis, Knoxville", "car-dependent communities that grew fast in the 1950s"],
            ["Downtown city cores", "City centers", "lost residents and shops as families moved to the suburbs"],
            ["Levittown", "New York", "the model mass-produced postwar suburb"],
        ],
    }
    geo = GEO_TASKS.get(code, "")
    geo_places = GEO_PLACES.get(code, [])  # curated only — no generic G-tag fallback
...
json.dump(content, open(f"{OUT}/unit8_content.json", "w"), ensure_ascii=False, indent=1)
json.dump(exit_tickets, open(f"{OUT}/unit8_exit_tickets.json", "w"), ensure_ascii=False, indent=1)
# NOTE: unit8_images.json is produced by build_unit2_images.py — do NOT clobber it here.
