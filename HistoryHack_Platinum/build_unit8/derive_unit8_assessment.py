#!/usr/bin/env python3
"""Derive analysis/unit8_assessment.json (Formative + Summative Form A/B) for the
...
ican = {s["standardCode"]: s for s in json.load(open(f"{WEB}/ican/unit-8.json"))}
ORDER = list(ican.keys())
dok = {}
for l in (1, 2, 3, 4):
    raw = json.load(open(f"{WEB}/questions/unit-8/dok-{l}.json"))
...
DISTINCT = {
 "US.71": ["consumerism", "affluent society", "credit", "advertising", "conformity", "suburb", "levittown", "gnp"],
 "US.72": ["baby boom", "boomer", "suburb", "dr. spock", "population", "postwar family"],
 "US.73": ["eisenhower", "interstate highway", "modern republicanism", "military-industrial", "highway act"],
 "US.74": ["automobile", "interstate", "highway", "suburb", "car culture", "drive-in", "motel"],
 "US.75": ["television", "mass media", "sitcom", "i love lucy", "broadcast", "advertising", "quiz show"],
 "US.76": ["rock and roll", "elvis", "sun records", "sam phillips", "memphis", "presley", "johnny cash", "rockabilly", "teenager"],
 "US.77": ["kennedy", "new frontier", "peace corps", "space program", "camelot", "space race"],
}
...
out = {"unit": "Unit 8", "formative": formative, "formA": formA, "formB": formB,
...
json.dump(out, open(f"{OUT}/unit8_assessment.json", "w"), ensure_ascii=False, indent=1)
