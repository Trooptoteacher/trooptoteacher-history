// Unit 8 Course Standard STUDENT WORKBOOK — docx-js, EXACT Unit 5 template (no deviation).
...
const C=JSON.parse(fs.readFileSync('analysis/unit8_content.json','utf8'));
const IMG=fs.existsSync('analysis/unit8_images.json')?JSON.parse(fs.readFileSync('analysis/unit8_images.json','utf8')):{};
const EXIT=fs.existsSync('analysis/unit8_exit_tickets.json')?JSON.parse(fs.readFileSync('analysis/unit8_exit_tickets.json','utf8')):{};
...
const HERO={file:'analysis/assets/hero.jpg', w:1200, h:1212,
  cap:'Astronaut Buzz Aldrin walks on the Moon during Apollo 11, July 20, 1969.',
  cred:'Neil Armstrong / NASA. Public domain.'};
...
const cover=[
  ...
  P(R('UNIT 8',{s:52,b:true,c:RED}),{align:AlignmentType.CENTER,spacing:{after:60}}),
  P(R('Postwar America, 1945–1963',{s:32,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:120}}),
  P(R('COURSE STANDARD EDITION',{s:26,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:60}}),
  P(R('US.71 – US.77   •   7 Tennessee U.S. History Standards   •   Grades 9–12',{s:22,c:INK}),{align:AlignmentType.CENTER,spacing:{after:140}}),
  ...coverHero(HERO),
  callout('TENNESSEE CONNECTION · MEMPHIS & THE BIRTH OF ROCK ’N’ ROLL',['Rock and roll was born in Tennessee. At Sun Records in Memphis, producer Sam Phillips recorded Elvis Presley, Johnny Cash, Carl Perkins, and Jerry Lee Lewis — blending blues and country into a sound that defined a generation of youth culture (US.76). Nashville’s WSM-TV carried the Grand Ole Opry into living rooms (US.75), while new interstates like I-40 reshaped the state (US.73–US.74).']),
...
const front=[
  ...
  P(R('U.S. History Hack™ — Unit 8: Postwar America (1945–1963), Course Standard Edition.',{s:22,b:true})),
  ...
  P(R('Framework stack. This workbook anchors to Tennessee U.S. History standards US.71–US.77 and the Social Studies Practices (SSP.01–SSP.06), and is designed on CAST UDL 3.0 and an MTSS support model.',{s:22})),
...
function block(code){
  ...
  out.push(callout('PREVIEW & PREDICT',['Which learning target will be hardest, and why? Connect it to the unit’s big question: who shared in the postwar prosperity, and who was left out?']));
  ...
  out.push(callout('BIG-QUESTION ORGANIZER — in postwar America, who shared in the prosperity and the new consumer culture, and who was left out?',['Jot quick notes from THIS standard: Who benefited? · Who bore the costs? · Who decided?']));
  out.push(...ruled(2));
...
const back=[
  ...
  callout('TENNESSEE CONNECTION · MEMPHIS, MUSIC, AND THE MODERN SOUTH',['Postwar Tennessee helped invent modern American culture. At Sun Records in Memphis, producer Sam Phillips blended Black rhythm-and-blues with white country music and launched Elvis Presley, Johnny Cash, Carl Perkins, and Jerry Lee Lewis — the sound of a new teenage culture (US.76). Nashville’s WSM-TV brought the Grand Ole Opry to television and helped make the city “Music City” (US.75). Off the airwaves, the interstate boom remade the state: I-40, I-65, I-24, and I-75 crossed Tennessee (US.73–US.74), fueling suburban growth around Nashville, Memphis, and Knoxville (US.71–US.72), while President Kennedy’s New Frontier targeted Appalachian poverty in East Tennessee (US.77).']),
  ...
  H('The Affluent Society — Who Shared the Boom, and Who Was Left Out',1),
  P(R('This section complicates the “happy days” image of the 1950s: postwar prosperity was real, but it reached some Americans far more than others. It is History Hack-authored instructional synthesis grounded in this unit’s sourced record — not a primary source.',{s:22})),
  ...[['The Suburban Boom','Cheap cars, new interstates, and easy credit built a consumer society (US.71, US.74) and a baby-boom generation raised in the suburbs (US.72). Television sold a shared national culture of family and prosperity (US.75).'],
    ['Left Out of the Dream','The suburban boom was largely closed to Black families by redlining and restrictive covenants, and it passed by the rural and Appalachian poor — the poverty Kennedy’s New Frontier later targeted in East Tennessee (US.77).'],
    ['A New Youth Culture','Teenagers became a force of their own. Rock and roll — born at Memphis’s Sun Records with Elvis and Johnny Cash (US.76) — crossed the color line in sound and unsettled the decade’s conformity.'],
    ['Conformity and Its Cracks','Beneath the prosperity ran pressures to conform, and quiet challenges to it — early stirrings of the civil rights movement and questions about who the American Dream was really for.']
  ...
  H('Unit Reflection',2),
  P(R('Return to the unit’s big question: in postwar America, who shared in the prosperity and the new consumer culture, and who was left out? Write a final claim with two pieces of evidence from different standards.',{s:22})),
...
// ================= ASSEMBLE =================
const header=new Header({children:[P(R('U.S. History Hack™ · Unit 8 · Course Standard Edition',{s:16,b:true,c:GOLD}),{align:AlignmentType.RIGHT,spacing:{after:0}})]});
const footer=new Footer({children:[new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:0},children:[
  R('U.S. History Hack™ · Unit 8 (Course Standard)   © 2026 TroopToTeacher Technologies LLC   |   Page ',{s:15,c:GREY}),
  new TextRun({children:[PageNumber.CURRENT],size:15,color:GREY,bold:true,font:FONT})]})]});
const doc=new Document({creator:'TroopToTeacher Technologies LLC',title:'U.S. History Hack — Unit 8 Course Standard Student Workbook (Pilot)',
  ...
  sections:[{properties:{page:{size:{width:12240,height:15840},margin:{top:1152,bottom:1152,left:1296,right:1296,header:720,footer:720}}},
    headers:{default:header},footers:{default:footer},children: SAMPLE ? [P(R(`SAMPLE — Unit 8 · first ${SAMPLE} standards (activities only, ruled-line writing) — for review`,{s:24,b:true,c:NAVY}),{align:AlignmentType.CENTER,spacing:{after:120}}), ...standards] : [...cover,...front,...standards,...back]}]});
const OUTFILE=SAMPLE?`deliverables/Unit8_SAMPLE_${SAMPLE}standards.docx`:'deliverables/Unit8_Student_Workbook_CourseStandard.docx';
